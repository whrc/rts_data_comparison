...
-   Figure out how to add tiles with multipolygons back into the analysis accurately (mask the tiles somehow)
-   Add median RTS IoU to table with thresholds
...
plot_prediction <- function(pid) {
  ...
  current_data <- polys %>%
    filter(id == pid) %>%
    select(imagery, id, rts_iou, pq_ci, pq_ci_size, bbox_geometry, rts_geometry, 
           val_geometry, pred_geometry)
...
rts_outlines <- st_read('./data/rts_polygons/rts_polygons_for_Yili_May_2022_v2.shp') %>%
  ...
  select(id = ID, region, status = Act_or_Gen, code_id, rts_area = Shape_Area, 
         Long) %>%
  mutate(rts_shape = as.numeric(
    4*pi*st_area(geometry)/st_perimeter(geometry)^2)
...
summary(maxar_rts_iou_nls)

# maxar_single_rts_iou_nls <- nls(rts_iou ~ Vmax * rts_area / (Km + rts_area),
#                start = list(Km = 50, Vmax = 2),
#                data = single_rts %>%
#                  filter(imagery == 'Maxar'))
# 
# maxar_single_rts_iou_boot <- nlsBoot(maxar_single_rts_iou_nls, 500000)
# saveRDS(maxar_single_rts_iou_nls, './data/models/maxar_single_rts_iou_nls.RData')
# saveRDS(maxar_single_rts_iou_boot, './data/models/maxar_single_rts_iou_boot.RData')

maxar_single_rts_iou_nls <- readRDS('./data/models/maxar_single_rts_iou_nls.RData')
maxar_single_rts_iou_boot <- readRDS('./data/models/maxar_single_rts_iou_boot.RData')
summary(maxar_single_rts_iou_nls)
```
...
summary(planet_rts_iou_nls)

# planet_single_rts_iou_nls <- nls(rts_iou ~ Vmax * rts_area / (Km + rts_area),
#                start = list(Km = 50, Vmax = 2),
#                data = single_rts %>%
#                  filter(imagery == 'Planet'))
# 
# planet_single_rts_iou_boot <- nlsBoot(planet_single_rts_iou_nls, 500000)
# saveRDS(planet_single_rts_iou_nls, './data/models/planet_single_rts_iou_nls.RData')
# saveRDS(planet_single_rts_iou_boot, './data/models/planet_single_rts_iou_boot.RData')

planet_single_rts_iou_nls <- readRDS('./data/models/planet_single_rts_iou_nls.RData')
planet_single_rts_iou_boot <- readRDS('./data/models/planet_single_rts_iou_boot.RData')
summary(planet_single_rts_iou_nls)
```
...
summary(sentinel_rts_iou_nls)

# sentinel_single_rts_iou_nls <- nls(rts_iou ~ Vmax * rts_area / (Km + rts_area),
#                start = list(Km = 50, Vmax = 2),
#                data = single_rts %>%
#                  filter(imagery == 'Sentinel-2'))
# 
# sentinel_single_rts_iou_boot <- nlsBoot(sentinel_single_rts_iou_nls, 500000)
# saveRDS(sentinel_single_rts_iou_nls, './data/models/sentinel_single_rts_iou_nls.RData')
# saveRDS(sentinel_single_rts_iou_boot, './data/models/sentinel_single_rts_iou_boot.RData')

sentinel_single_rts_iou_nls <- readRDS('./data/models/sentinel_single_rts_iou_nls.RData')
sentinel_single_rts_iou_boot <- readRDS('./data/models/sentinel_single_rts_iou_boot.RData')
summary(sentinel_single_rts_iou_nls)
```

Table for nls model summaries

```{r}
rts_iou_summary <- tidy(maxar_rts_iou_nls) %>%
  mutate(Imagery = 'Maxar') %>%
  rbind(tidy(planet_rts_iou_nls) %>%
          mutate(Imagery = 'Planet')) %>%
  rbind(tidy(sentinel_rts_iou_nls) %>%
          mutate(Imagery = 'Sentinel-2')) %>%
...
single_rts_iou_summary <- tidy(maxar_single_rts_iou_nls) %>%
  mutate(Imagery = 'Maxar') %>%
  rbind(tidy(planet_single_rts_iou_nls) %>%
          mutate(Imagery = 'Planet')) %>%
  rbind(tidy(sentinel_single_rts_iou_nls) %>%
          mutate(Imagery = 'Sentinel-2')) %>%
  mutate(across(estimate:p.value, ~ round(.x, 3)),
         p.value = case_when(p.value == 0 ~ '<0.001',
                             p.value > 0 ~ as.character(p.value))) %>%
...
# write.csv(single_rts_iou_summary,
#           './tables/single_rts_iou_size_nls.csv',
#           row.names = FALSE)

knitr::kable(single_rts_iou_summary)
```
...
# maxar_rts_iou_predict <- my_nlsBootPredict(maxar_rts_iou_boot, newdata = newdata, interval = 'prediction')
# saveRDS(maxar_rts_iou_predict, './data/models/maxar_rts_iou_predict.RData')

maxar_rts_iou_predict <- readRDS('./data/models/maxar_rts_iou_predict.RData')
maxar_rts_iou_ci <- newdata %>%
  cbind(maxar_rts_iou_predict %>%
          as_tibble()) %>%
  select(rts_area, prediction = Median, lwr = `25%`, upr = `75%`)

# maxar_single_rts_iou_predict <- my_nlsBootPredict(maxar_single_rts_iou_boot, newdata = newdata2, interval = 'prediction')
# saveRDS(maxar_single_rts_iou_predict, './data/models/maxar_single_rts_iou_predict.RData')

maxar_single_rts_iou_predict <- readRDS('./data/models/maxar_single_rts_iou_predict.RData')
maxar_single_rts_iou_ci <- newdata2 %>%
  cbind(maxar_single_rts_iou_predict %>%
          as_tibble()) %>%
  select(rts_area, prediction = Median, lwr = `25%`, upr = `75%`)
```
...
# planet_rts_iou_predict <- my_nlsBootPredict(planet_rts_iou_boot, newdata = newdata, interval = 'prediction')
# saveRDS(planet_rts_iou_predict, './data/models/planet_rts_iou_predict.RData')

planet_rts_iou_predict <- readRDS('./data/models/planet_rts_iou_predict.RData')
planet_rts_iou_ci <- newdata %>%
  cbind(planet_rts_iou_predict %>%
          as_tibble()) %>%
  select(rts_area, prediction = Median, lwr = `25%`, upr = `75%`)

# planet_single_rts_iou_predict <- my_nlsBootPredict(planet_single_rts_iou_boot, newdata = newdata2, interval = 'prediction')
# saveRDS(planet_single_rts_iou_predict, './data/models/planet_single_rts_iou_predict.RData')

planet_single_rts_iou_predict <- readRDS('./data/models/planet_single_rts_iou_predict.RData')
planet_single_rts_iou_ci <- newdata2 %>%
  cbind(planet_single_rts_iou_predict %>%
          as_tibble()) %>%
  select(rts_area, prediction = Median, lwr = `25%`, upr = `75%`)
```
...
# sentinel_rts_iou_predict <- my_nlsBootPredict(sentinel_rts_iou_boot, newdata = newdata, interval = 'prediction')
# saveRDS(sentinel_rts_iou_predict, './data/models/sentinel_rts_iou_predict.RData')

sentinel_rts_iou_predict <- readRDS('./data/models/sentinel_rts_iou_predict.RData')
sentinel_rts_iou_ci <- newdata %>%
  cbind(sentinel_rts_iou_predict %>%
          as_tibble()) %>%
  select(rts_area, prediction = Median, lwr = `25%`, upr = `75%`)

# sentinel_single_rts_iou_predict <- my_nlsBootPredict(sentinel_single_rts_iou_boot, newdata = newdata2, interval = 'prediction')
# saveRDS(sentinel_single_rts_iou_predict, './data/models/sentinel_single_rts_iou_predict.RData')

sentinel_single_rts_iou_predict <- readRDS('./data/models/sentinel_single_rts_iou_predict.RData')
sentinel_single_rts_iou_ci <- newdata2 %>%
  cbind(sentinel_single_rts_iou_predict %>%
          as_tibble()) %>%
  select(rts_area, prediction = Median, lwr = `25%`, upr = `75%`)
```
...
rts_iou_predictions <- rbind(
  my_nlsBootPredict(maxar_rts_iou_boot, 
                    polys %>%
                      as_tibble() %>%
                      filter(imagery == 'Maxar'),
                    interval = 'prediction'),
  my_nlsBootPredict(planet_rts_iou_boot, 
                    polys %>%
                      as_tibble() %>%
                      filter(imagery == 'Planet'),
                    interval = 'prediction'),
  my_nlsBootPredict(sentinel_rts_iou_boot, 
                    polys %>%
                      as_tibble() %>%
                      filter(imagery == 'Sentinel-2'),
                    interval = 'prediction')
...
polys <- polys %>%
  mutate(
    mean_iou_pred = mean_iou_predictions[, 1],
    mean_iou_pred_lwr = mean_iou_predictions[, 2],
    mean_iou_pred_upr = mean_iou_predictions[, 3],
    rts_iou_pred = rts_iou_predictions[, 1],
    rts_iou_pred_lwr = rts_iou_predictions[, 2],
    rts_iou_pred_upr = rts_iou_predictions[, 3]
  )

single_rts_iou_predictions <- rbind(
  my_nlsBootPredict(maxar_single_rts_iou_boot, 
                    single_rts %>%
                      as_tibble() %>%
                      filter(imagery == 'Maxar'),
                    interval = 'prediction'),
  my_nlsBootPredict(planet_single_rts_iou_boot, 
                    single_rts %>%
                      as_tibble() %>%
                      filter(imagery == 'Planet'),
                    interval = 'prediction'),
  my_nlsBootPredict(sentinel_single_rts_iou_boot, 
                    single_rts %>%
                      as_tibble() %>%
                      filter(imagery == 'Sentinel-2'),
                    interval = 'prediction')
...
single_rts <- single_rts %>%
  mutate(
    rts_iou_pred = single_rts_iou_predictions[, 1],
    rts_iou_pred_lwr = single_rts_iou_predictions[, 2],
    rts_iou_pred_upr = single_rts_iou_predictions[, 3]
  )
```
...
single_rts_iou_ci <- maxar_single_rts_iou_ci %>%
  mutate(imagery = 'Maxar') %>%
  rbind(planet_single_rts_iou_ci %>%
          mutate(imagery = 'Planet')) %>%
  rbind(sentinel_single_rts_iou_ci %>%
          mutate(imagery = 'Sentinel-2'))
...
ggplot(polys, 
       aes(x = rts_area, y = rts_iou, 
           color = imagery, 
           shape = st_geometry_type(val_geometry), 
           alpha = st_geometry_type(val_geometry))) +
  geom_point(size = 1) +
...
```{r}
ggplot(polys, 
      aes(x = rts_area, y = rts_iou, color = imagery, shape = st_geometry_type(val_geometry))) +
  geom_point(size = 1) +
  scale_x_continuous(name = expression('RTS Area (m'^2*')')) +
  scale_y_continuous(name = 'RTS IoU') +
  scale_color_manual(values = imagery_colors,
...
ggplot(polys, 
  ...
  scale_color_manual(values = imagery_colors,
                     labels = imagery_labels) +
  scale_shape_manual(values = c(16, 1)) +
  facet_grid(. ~ st_geometry_type(val_geometry)) +
  theme(legend.title = element_blank())
...
maxar_single_d1 <- D(expression(y = 0.8459141 * x / (3127.1810378 + x)), name = 'x')
planet_single_d1 <- D(expression(y = 0.7716229 * x / (2744.6832193 + x)), name = 'x')
sentinel_single_d1 <- D(expression(y = 0.8326861 * x / (5833.1242848 + x)), name = 'x')
...
maxar_single_est <- estimate_grid %>%
  mutate(slope_est = eval_expression(maxar_single_d1, rts_area))
planet_single_est <- estimate_grid %>%
  mutate(slope_est = eval_expression(planet_single_d1, rts_area))
sentinel_single_est <- estimate_grid %>%
  mutate(slope_est = eval_expression(sentinel_single_d1, rts_area))
...
maxar_single_cross <- maxar_single_est %>%
  slice(which.min(abs(1e-05-slope_est))) %>%
  select(rts_area) %>%
  as.numeric() # find closest to 1e-05
planet_single_cross <- planet_single_est %>%
  slice(which.min(abs(1e-05-slope_est))) %>%
  select(rts_area) %>%
  as.numeric() # find closest to 1e-05
sentinel_single_cross <- sentinel_single_est %>%
  slice(which.min(abs(1e-05-slope_est))) %>%
  select(rts_area) %>%
  as.numeric() # find closest to 1e-05

size_single_thresholds <- tibble(imagery = c('Maxar', 'Planet', 'Sentinel-2'),
                                 rts_threshold = c(maxar_single_cross, 
                                                   planet_single_cross, 
                                                   sentinel_single_cross))
...
single_rts_iou_thresholds <- single_rts %>%
  as_tibble() %>%
  group_by(imagery) %>%
  summarise(rts_iou_mean = mean(rts_iou),
            rts_sd = sd(rts_iou)) %>%
...
single_rts <- single_rts %>%
  mutate(
    size_threshold = case_when(
      imagery == 'Maxar' ~ 
        filter(size_single_thresholds, imagery == 'Maxar')$rts_threshold,
      imagery == 'Planet' ~ 
        filter(size_single_thresholds, imagery == 'Planet')$rts_threshold,
      imagery == 'Sentinel-2' ~ 
        filter(size_single_thresholds, imagery == 'Sentinel-2')$rts_threshold
    ),
    iou_threshold = case_when(
      imagery == 'Maxar' ~ 
        filter(single_rts_iou_thresholds, imagery == 'Maxar')$rts_iou_threshold,
      imagery == 'Planet' ~ 
        filter(single_rts_iou_thresholds, imagery == 'Planet')$rts_iou_threshold,
      imagery == 'Sentinel-2' ~ 
        filter(single_rts_iou_thresholds, imagery == 'Sentinel-2')$rts_iou_threshold
    ),
    pq_ci = factor(
      case_when(
        rts_iou > rts_iou_pred_upr & rts_iou > iou_threshold ~ 'High',
...
```{r}
single_rts_performance_summary <- polys %>%
  as_tibble() %>%
  filter(st_geometry_type(val_geometry) == 'POLYGON') %>%
  summarise(rts_iou_median = median(rts_iou),
            .by = 'imagery')

min_rts_area <- single_rts_iou_ci %>%
  left_join(single_rts_performance_summary,
            by = 'imagery') %>%
...
input_data <- input_data %>%
  left_join(polys %>%
              ...
              select(id, imagery, yg, rts_area, rts_shape, rts_iou, mean_iou, 
                     mean_iou_v2, pq_ci, pq_ci_size, pq_ci_offset),
            by = c('id', 'imagery'))
```
