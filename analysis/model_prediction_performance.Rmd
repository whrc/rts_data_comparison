...
polys <- polys %>%
  mutate(
    expected_performance_th = case_when(
      ...
      TRUE ~ 'Expected'),
    expected_performance_ci = case_when(
...
polys <- polys %>%
  mutate(
    ...
    expected_performance_ci_size = case_when(
...
ggplot(polys, 
       aes(x = rts_area, 
           ...
           shape = imagery)) +
  geom_point(size = 1) +
  geom_vline(xintercept = filter(size_thresholds, imagery == 'Maxar')$rts_threshold,
             color = imagery_colors[1]) +
  geom_vline(xintercept = filter(size_thresholds, imagery == 'Planet')$rts_threshold,
             color = imagery_colors[2]) +
  geom_vline(xintercept = filter(size_thresholds, imagery == 'Sentinel-2')$rts_threshold,
             color = imagery_colors[3]) +
  scale_x_continuous(name = expression('RTS Area (m'^2*')')) +
  scale_y_continuous(name = 'IoU') +
  theme_bw() +
  theme(legend.title = element_blank()) + 
