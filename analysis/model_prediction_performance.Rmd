...
```{r, include = FALSE}
zonal_mean <- function(rast) {
  
  # unify layer names
  names(rast) <- c('r', 'g', 'b', names(rast)[4:9])
  ...
  # replace NAs with 0 in RTS label layer
  rast[[9]][is.na(rast[['RTSlabel']])] <- 0
  
  stats <- zonal(subset(rast, 1:8), 
                 z = subset(rast, 'RTSlabel'), 
                 fun = mean) %>%
    mutate(id = id)
...
```{r}
zonal_stats <- maxar_input %>%
  map_dfr(~ zonal_mean(.x)) %>%
  mutate(imagery = 'Maxar') %>%
  ...
  rbind.data.frame(sentinel_input %>%
                     ...
                     mutate(imagery = 'Sentinel-2')) %>%
  group_by(imagery) %>%
  mutate(rts_class = factor(case_when(RTSlabel == 0 ~ 'bg',
                                      RTSlabel == 1 ~ 'rts')),
         r = r/max(r, na.rm = TRUE),
         g = g/max(g, na.rm = TRUE),
         b = b/max(b, na.rm = TRUE),
         luminance = 0.299*r + 0.587*g + 0.114*b,
         planetNIR = planetNIR/max(planetNIR, na.rm = TRUE)) %>% # luminance equation from: https://stackoverflow.com/questions/596216/formula-to-determine-perceived-brightness-of-rgb-color
  select(id, imagery, rts_class,
         r, g, b, luminance, nir = planetNIR, 
         ndvi, ndwi, 
         relative_elevation, shaded_relief) %>%
  pivot_wider(names_from = 'rts_class', values_from = c(r:shaded_relief)) %>%
  mutate(r_delta = (r_rts - r_bg)/r_bg,
         b_delta = (b_rts - b_bg)/b_bg,
         g_delta = (g_rts - g_bg)/g_bg,
         luminance_delta = (luminance_rts - luminance_bg)/luminance_bg,
         nir_delta = (nir_rts - nir_bg)/nir_bg,
         ndvi_delta = (ndvi_rts - ndvi_bg)/ndvi_bg,
         ndwi_delta = (ndwi_rts - ndwi_bg)/ndwi_bg,
         relative_elevation_delta = (relative_elevation_rts - relative_elevation_bg)/relative_elevation_bg,
         shaded_relief_delta = (shaded_relief_rts - shaded_relief_bg)/shaded_relief_bg) %>%
  right_join(polys %>%
               as_tibble() %>%
...
zonal_stats_limits <- zonal_stats %>%
  select(id:shaded_relief_delta, expected_performance_ci_size) %>%
  pivot_longer(r_bg:shaded_relief_delta,  names_to = 'variable', values_to = 'value') %>%
group_by(imagery, expected_performance_ci_size, variable) %>%
...
```{r}
ggplot(zonal_stats, aes(x = r_rts, color = imagery)) +
  geom_freqpoly() +
  scale_color_manual(values = imagery_colors) +
  theme_bw()
```
...
red_fig <- ggplot(zonal_stats, 
  ...
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'r_rts') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
red_fig
...
green_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'g_rts') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
green_fig
...
blue_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'b_rts') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
blue_fig

luminance_fig <- ggplot(zonal_stats, 
       aes(x = expected_performance_ci_size, 
           y = luminance_rts, 
           color = expected_performance_ci_size)) +
  geom_boxplot(outlier.shape = NA) +
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'luminance_rts') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
luminance_fig
...
nir_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'nir_rts') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
nir_fig
...
ndvi_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'ndvi_rts') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
ndvi_fig
...
ndwi_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'ndwi_rts') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
ndwi_fig
...
elev_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'relative_elevation_rts') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
elev_fig
...
relief_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'shaded_relief_rts') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
relief_fig
```
...
red_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'r_delta') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
red_fig
...
green_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'g_delta') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
green_fig
...
blue_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'b_delta') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
blue_fig
...
luminance_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'luminance_delta') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
luminance_fig
...
nir_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'nir_delta') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
nir_fig
...
ndvi_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'ndvi_delta') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
ndvi_fig
...
ndwi_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'ndwi_delta') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
ndwi_fig
...
elev_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'relative_elevation_delta') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
elev_fig
...
relief_fig <- ggplot(zonal_stats, 
  ...
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(ylim = zonal_stats_limits %>%
                    filter(variable == 'shaded_relief_delta') %>% # rlang::as_name(??$mapping$y)
                    select(lwr, upr) %>%
                    as_vector())
relief_fig
```
