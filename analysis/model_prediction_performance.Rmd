...
-   Think more about NDWI and sr zonal stats. What exactly do the sr values indicate?
...
### my_nlsBootPredict (wity my desired quantiles)
...
plot_prediction <- function(pid) {
  ...
  current_data <- polys %>%
    filter(id == pid) %>%
    select(imagery, id, rts_iou, pq_ci, pq_ci_size, bbox_geometry, val_geometry, 
           pred_geometry)
...
# get polygon data
current_data <- polys %>%
  filter(pq_ci_size == 'High' & imagery == 'Maxar') %>%
  select(imagery, id, rts_iou, pq_ci, pq_ci_size, bbox_geometry, val_geometry, 
         pred_geometry)
...
# get polygon data
current_data <- polys %>%
  filter(pq_ci_size == 'Low' & imagery == 'Maxar') %>%
  select(imagery, id, rts_iou, pq_ci, pq_ci_size, bbox_geometry, val_geometry, 
         pred_geometry)
...
# get polygon data
current_data <- polys %>%
  filter(pq_ci_size == 'High' & imagery == 'Planet') %>%
  select(imagery, id, rts_iou, pq_ci, pq_ci_size, bbox_geometry, val_geometry, 
         pred_geometry)
...
# get polygon data
current_data <- polys %>%
  filter(pq_ci_size == 'Low' & imagery == 'Planet') %>%
  select(imagery, id, rts_iou, pq_ci, pq_ci_size, bbox_geometry, val_geometry, 
         pred_geometry)
...
# get polygon data
current_data <- polys %>%
  filter(pq_ci_size == 'High' & imagery == 'Sentinel-2') %>%
  select(imagery, id, rts_iou, pq_ci, pq_ci_size, bbox_geometry, val_geometry, 
         pred_geometry)
...
# get polygon data
current_data <- polys %>%
  filter(pq_ci_size == 'Low' & imagery == 'Sentinel-2') %>%
  select(imagery, id, rts_iou, pq_ci, pq_ci_size, bbox_geometry, val_geometry, 
         pred_geometry)
...
input_data <- input_data %>%
  left_join(polys %>%
              as_tibble() %>%
              select(id, imagery, region, rts_area, rts_iou, mean_iou, mean_iou_v2, 
                     pq_ci, pq_ci_size, pq_ci_size_offset),
            by = c('id', 'imagery'))

zonal_stats <- input_data %>%
  group_by(imagery, id, region, rts_class, rts_area, rts_iou, mean_iou, mean_iou_v2, 
           pq_ci, pq_ci_size, pq_ci_size_offset) %>%
...
- Brighter RTS against a darker background are predicted better. Maxar: poorly predicted RTS may tend to be underexposed (whole image is dark). Planet: poorly predicted RTS may tend to be overexposed (whole image is bright). Possible this is related to seasonality?
...
-   NDVI (mean) figures indicate that RTS may be predicted poorly where background cells have low NDVI/plant growth. I.e. it's harder for the model to find the RTS where there's not much plant growth across the landscape (or if it's early/late season imagery?). Also important to note is that NDVI is coming from Sentinel-2, so is at lower resolution than than the RGB imagery. This could be introducing errors leading to non-significant relationships.

- RTS are also predicted better where the background has variable plant growth and the RTS feature has spatially consistent plant growth.
...
```{r}
ggplot(polys, 
       aes(x = rts_shape, y = rts_area, color = rts_iou)) +
  geom_point() +
...
# the r2 is about the same for Planet with rts_iou ~ rts_area_log10
# or rts_iou ~ rts_area_log10 * rts_shape
# to allow comparison of the imagery types, I will stick with the more complex
# model that was chosen as the best for both Maxar and Sentinel-2
