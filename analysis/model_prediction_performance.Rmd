...
```{r}
input_ids <- tibble(maxar = map_chr(maxar_input, ~ get_rast_id(.x)),
                    planet = map_chr(planet_input, ~ get_rast_id(.x)),
                    sentinel = map_chr(sentinel_input, ~ get_rast_id(.x)))

if (all(input_ids$maxar == input_ids$planet) & all(input_ids$maxar == input_ids$sentinel)) {
  input_ids <- input_ids %>%
    select(id = maxar)
}
```
...
for (i in 1:69) {
  ...
  # get index of polygon ID in the input data
  idx <- which(input_ids$id == current_id)
  
  # prep RGB data
  rgb_df <- stretch(subset(maxar_input[[idx]], 1:3),
                     minv = 0,
                     maxv = 1,
                     minq = 0.01,
                     maxq = 1)%>%
    as.data.frame(stretch,
                  xy = TRUE) %>%
    rename(r = 3, g = 4, b = 5) %>%
    filter(!(is.na(r) | is.na(g) | is.na(b))) %>%
    mutate(color.hex = factor(rgb(r, g, b, maxColorValue = 1)),
           imagery = 'Maxar') %>%
    rbind.data.frame(stretch(subset(planet_input[[idx]], 1:3),
                     minv = 0,
                     maxv = 1,
                     minq = 0.01,
                     maxq = 1)%>%
    ...
    mutate(color.hex = factor(rgb(r, g, b, maxColorValue = 1)),
           imagery = 'Planet')) %>%
    rbind.data.frame(stretch(subset(sentinel_input[[idx]], 1:3),
                     minv = 0,
                     maxv = 1,
                     minq = 0.01,
                     maxq = 1)%>%
  ...
  print(ggplot(data = current_data) +
          geom_raster(data = rgb_df, 
                      ...
                      inherit.aes = FALSE) +
          scale_fill_manual(values = levels(rgb_df$color.hex),
                            guide = 'none') +
          ...
          geom_sf(aes(geometry = pred_geometry),
                  ...
                  fill = 'transparent',
                  linewidth = 0.5) +
          facet_grid(.~ imagery + paste('mIoU:', round(mean_iou, 2))) +
          ggtitle(paste('ID:', current_id)) +
          theme(axis.title = element_blank()) +
          theme_bw()
          )
  
  rm(current_data, current_id, idx, rgb_df)
...
my_model <- function(df) {
  linear_reg() %>%
  fit(r_mean ~ expected_performance_ci_size, 
            data = df)
model_output <- tidy(model)
return(model_output)
}
# contrasts <- as_tibble(emmeans(model,
...
```{r, warning = FALSE}
for (var in colnames(zonal_stats %>%
                          ungroup() %>%
                          select(r_mean:sr_sd))) {
