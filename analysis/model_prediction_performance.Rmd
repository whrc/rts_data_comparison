...
- finish figure with zonal stats results 
...
``` {r}
format_water_mask <- function(x, y) {
  formatted_mask <- x %>%
    classify(matrix(c(0, 1,
                      0.5, NA,
                      1, NA),
                    ncol = 2,
                    byrow = TRUE)) %>%
    disagg(5) %>%
    project(y, align = TRUE) %>%
    crop(y)
  
  names(formatted_mask) <- c('water')

  return(formatted_mask)
}
```

### remove_rts_from_water_mask

```{r}
remove_rts_from_water_mask <- function(x, y) {
  x[y[['validation']] == 1] <- 1
  return(x)
}
```
...
get_rast_id <- function(rast) {
  id <- str_extract(first(str_split(last(str_split(sources(rast), 
                                       '/')[[1]]), 
                        '\\.')[[1]]),
                    '0{10,}[:alnum:]+')
  return(id)
...
input_as_df <- function(input, pred) {
  ...
  # unify layer names
  names(input) <- c('r', 'g', 'b', 'ndvi', 'elev', 'sr', 'nir', 'ndwi', 'rts_class', 'water', 'prediction')
  ...
  df <- input %>%
    ...
    mutate(id = current_id,
           ...
           rts_class_water = factor(
             case_when(rts_class == 'rts' ~ 'rts',
                       rts_class == 'bg' & water == 1 ~ 'bg',
                       rts_class == 'bg' & is.na(water) ~ 'water'),
             levels = c('rts', 'bg', 'water')
           ),
...
### plot_pred_single_band

```{r}
plot_pred_single_band <- function(pid, input_layer) {
  ...
  figure <- ggplot(data = current_polys) +
    geom_raster(data = current_rast,
                aes(x = x, y = y, fill = .data[[input_layer]]),
                inherit.aes = FALSE) +
    scale_fill_viridis(name = input_layer) +
    geom_sf(aes(geometry = val_geometry,
                color = 'Validation',
                linewidth = 'Validation'),
            fill = 'transparent') +
...
```{r}
plot_pred_rgb_2 <- function(pid, imagery_types, dims) {
...
plot_zonal_stats <- function(x_var, y_var, contrasts, limits) {
  ...
  rts_fig <- ggplot(zonal_stats %>%
    ...
    geom_signif(data = contrasts %>%
                  filter(p_value <= 0.1 & variable == y_var & rts_class == 'RTS'),
  ...
  delta_bg_fig <- ggplot(zonal_stats %>%
                        filter(rts_class == 'Delta BG'), 
                      aes(x = .data[[x_var]], 
                          y = .data[[y_var]], 
                          color = .data[[x_var]])) +
    geom_boxplot(
    ...
    geom_signif(data = contrasts %>%
                  filter(p_value <= 0.1 & variable == y_var & rts_class == 'Delta BG'),
                aes(x = x_min, y = y, xend = x_max, yend = y,
                    annotation = star_label, group = contrast),
                stat="identity",
                manual = TRUE,
    ...
    scale_color_manual(values = pq_colors,
                       guide = 'none') +
    facet_grid("Delta ~ '(RTS - BG)'" ~ imagery, labeller = label_parsed) +
    theme(axis.title.x = element_blank(),
          plot.margin = margin(0, 0, 0, 0, "pt")) +
    coord_cartesian(ylim = limits %>%
                      filter(variable == y_var & rts_class == 'Delta BG') %>%
                      ungroup() %>%
                      select(lwr, upr) %>%
                      as_vector())
  ...
  bg_fig <- ggplot(zonal_stats %>%
    ...
    geom_signif(data = contrasts %>%
                  filter(p_value <= 0.1 &variable == y_var & rts_class == 'Background'),
                aes(x = x_min, y = y, xend = x_max, yend = y,
                    annotation = star_label, group = contrast),
                stat="identity",
                manual = TRUE,
    ...
    water_fig <- ggplot(zonal_stats %>%
                     filter(rts_class == 'Water'), 
                   aes(x = .data[[x_var]], 
                       y = .data[[y_var]], 
                       color = .data[[x_var]])) +
    geom_boxplot(
    ...
    geom_signif(data = contrasts %>%
                  filter(p_value <= 0.1 &variable == y_var & rts_class == 'Water'),
                aes(x = x_min, y = y, xend = x_max, yend = y,
                    annotation = star_label, group = contrast),
                stat="identity",
                manual = TRUE,
    ...
    theme(plot.margin = margin(0, 0, 0, 0, "pt"))
    coord_cartesian(ylim = limits %>%
                      filter(variable == y_var & rts_class == 'Water') %>%
                      ungroup() %>%
                      select(lwr, upr) %>%
                      as_vector())

  delta_water_fig <- ggplot(zonal_stats %>%
                        filter(rts_class == 'Delta Water'), 
                      aes(x = .data[[x_var]], 
                          y = .data[[y_var]], 
                          color = .data[[x_var]])) +
    geom_boxplot(
    ...
    geom_signif(data = contrasts %>%
                  filter(p_value <= 0.1 & variable == y_var & rts_class == 'Delta Water'),
                aes(x = x_min, y = y, xend = x_max, yend = y,
                    annotation = star_label, group = contrast),
                stat="identity",
                manual = TRUE,
    ...
    scale_color_manual(values = pq_colors,
                       guide = 'none') +
    facet_grid("Delta ~ '(RTS - Water)'" ~ imagery, labeller = label_parsed) +
    theme(axis.title.x = element_blank(),
          plot.margin = margin(0, 0, 0, 0, "pt")) +
    coord_cartesian(ylim = limits %>%
                      filter(variable == y_var & rts_class == 'Delta Water') %>%
                      ungroup() %>%
                      select(lwr, upr) %>%
                      as_vector())
...
```{r, warning = FALSE}
plot_zonal_stats_2 <- function(x_var, y_var, contrasts, limits) {
...
```{r, warning = FALSE}
plot_zonal_stats_3 <- function(x_var, y_vars, contrasts, limits) {
  
  predictor_names <- case_when(
    y_vars == 'r_mean' ~ "expression('Red')",
    y_vars == 'g_mean' ~ "expression('Green')",
    y_vars == 'b_mean' ~ "expression('Blue')",
    y_vars == 'lum_mean' ~ "expression('Luminance')",
    y_vars == 'nir_mean' ~ "expression('NIR')",
    y_vars == 'ndvi_mean' ~ "expression('NDVI')",
    y_vars == 'ndwi_mean' ~ "expression('NDWI')",
    y_vars == 'elev_mean' ~ "expression('Relative Elevation')",
    y_vars == 'sr_mean' ~ "expression('Shaded Relief')",
    y_vars == 'r_sd' ~ "expression(sigma ~ 'Red')",
    y_vars == 'g_sd' ~ "expression(sigma ~ 'Green')",
    y_vars == 'b_sd' ~ "expression(sigma ~ 'Blue')",
    y_vars == 'lum_sd' ~ "expression(sigma ~ 'Luminance')",
    y_vars == 'nir_sd' ~ "expression(sigma ~ 'NIR')",
    y_vars == 'ndvi_sd' ~ "expression(sigma ~ 'NDVI')",
    y_vars == 'ndwi_sd' ~ "expression(sigma ~ 'NDWI')",
    y_vars == 'elev_sd' ~ "expression(sigma ~ 'Relative Elevation')",
    y_vars == 'sr_sd' ~ "expression(sigma ~ 'Shaded Relief')"
  )
  ...
  rts_figs <- map2(
    ...
    ~ ggplot(zonal_stats %>%
      ...
      geom_signif(
        data = contrasts %>%
          filter(p_value <= 0.1 & variable == .x & rts_class == 'RTS'),
        aes(x = x_min, y = y, xend = x_max, yend = y,
            annotation = star_label, group = contrast),
        stat="identity",
        manual = TRUE,
      ...
      scale_y_continuous(eval(parse(text = .y))) +
      scale_color_manual(values = pq_colors,
                         guide = 'none') +
      facet_grid(rts_class ~ imagery) +
      theme(axis.title.x = element_blank(),
            plot.margin = margin(0, 0, 0, 0, "pt")) +
      coord_cartesian(ylim = limits %>%
                        filter(variable == .x & rts_class == 'RTS') %>%
                        ungroup() %>%
                        select(lwr, upr) %>%
                        as_vector())
  )
  ...
  delta_figs <- map2(
    ...
    geom_boxplot(
      ...
      geom_signif(
        data = contrasts %>%
          filter(p_value <= 0.1 & variable == .x & rts_class == 'Delta BG'),
        aes(x = x_min, y = y, xend = x_max, yend = y,
            annotation = star_label, group = contrast),
        stat="identity",
        manual = TRUE,
    ...
    theme(axis.title.x = element_blank(),
          plot.margin = margin(0, 0, 0, 0, "pt")) +
    coord_cartesian(ylim = limits %>%
                      filter(variable == .x & rts_class == 'Delta BG') %>%
                      ungroup() %>%
                      select(lwr, upr) %>%
                      as_vector())
  )
  ...
  bg_figs <- map2(
    ...
    geom_boxplot(
      ...
      geom_signif(
        data = contrasts %>%
          filter(p_value <= 0.1 & variable == .x & rts_class == 'Background'),
        aes(x = x_min, y = y, xend = x_max, yend = y,
            annotation = star_label, group = contrast),
        stat="identity",
        manual = TRUE,
    ...
    scale_color_manual(values = pq_colors,
                       guide = 'none') +
    facet_grid("Background" ~ imagery, labeller = label_parsed) +
    theme(axis.title.x = element_blank(),
          plot.margin = margin(0, 0, 0, 0, "pt")) +
    coord_cartesian(ylim = limits %>%
                      filter(variable == .x & rts_class == 'Background') %>%
                      ungroup() %>%
                      select(lwr, upr) %>%
                      as_vector())
  )
...
names(maxar_preds) <- pred_ids
maxar_preds_zscore <- maxar_preds_zscore[indices]
names(maxar_preds_zscore) <- pred_ids
...
# filter to only include prediction tiles
...
water_files <- list.files(
  paste0(loc_dir, '/sentinel/water_mask'),
  full.names = TRUE,
  pattern = 'WorldView'
)
...
ids <- map_chr(maxar_water, ~ get_rast_id(.x))
names(maxar_water) <- ids

maxar_water <- map2(
  maxar_water,
  maxar_input,
  ~ format_water_mask(.x, .y)
)

maxar_water <- map2(
  maxar_water,
  maxar_preds,
  ~ remove_rts_from_water_mask(.x, .y)
)

# add mask to input data
maxar_input <- map2(maxar_input,
                    maxar_water,
                    ~ c(.x, .y))
```
...
#       ~ plotRGB(
#           mask(.x, .y),
#           r = 1, g = 2, b = 3, stretch = 'lin', colNA = 'red')
# )
```
...
ids <- map_chr(planet_water, ~ get_rast_id(.x))
names(planet_water) <- ids
...
# add mask to input data
planet_input <- map2(planet_input,
                     planet_water,
                     ~ c(.x, .y))
```
...
ids <- map_chr(sentinel_water, ~ get_rast_id(.x))
...
names(sentinel_water) <- c('water')

# add mask to input data
sentinel_input <- map2(sentinel_input,
                       sentinel_water,
                       ~ c(.x, .y))
...
# st_write(tiles,
#          './data/rts_polygons/all_tiles.shp')
...
input_data <- pmap(list(x = maxar_input, 
  ...
  select(imagery, id, rts_class, rts_class_water, x, y, 
...
### Investigate RTS vs. Background Coverage

```{r, fig.height = 3, fig.width = 3}
polys <- polys %>%
  mutate(rts_coverage = as.numeric(st_area(rts_geometry))/as.numeric(st_area(bbox_geometry)))

ggplot(polys %>%
         filter(imagery == 'WorldView'), 
       aes(x = rts_coverage)) +
  geom_histogram(binwidth = 0.05, boundary = 0) +
  scale_x_continuous(name = 'RTS Coverage of Tile (%)',
                     labels = ~.x*100) +
  scale_y_continuous(name = 'Tile Count')

# ggsave('./figures/rts_coverage.pdf',
#        height = 3, 
#        width = 3)
# ggsave('./figures/rts_coverage.jpg',
#        height = 3, 
#        width = 3)
```
...
zscore <- function(vector) {
  (vector - mean(vector, na.rm = TRUE))/sd(vector, na.rm = TRUE)
}

zonal_stats <- input_data %>%
  filter(mask == 1) %>%
  mutate(across(r:sr, zscore),
                .by = imagery) %>%
  summarise(across(r:sr, mean, .names = "{.col}_mean"),
            across(r:sr, sd, .names = "{.col}_sd"),
            .by = c(imagery, id, region, rts_class_water, rts_area, rts_shape, 
                    rts_iou, mean_iou, mean_iou_v2, pq_ci, pq_ci_size)) %>%
  pivot_wider(names_from = 'rts_class_water', 
              values_from = c(r_mean:sr_sd)) %>%
  mutate(r_mean_deltaBG = (r_mean_rts - r_mean_bg),
         g_mean_deltaBG = (g_mean_rts - g_mean_bg),
         b_mean_deltaBG = (b_mean_rts - b_mean_bg),
         lum_mean_deltaBG = (lum_mean_rts - lum_mean_bg),
         nir_mean_deltaBG = (nir_mean_rts - nir_mean_bg),
         ndvi_mean_deltaBG = (ndvi_mean_rts - ndvi_mean_bg),
         ndwi_mean_deltaBG = (ndwi_mean_rts - ndwi_mean_bg),
         elev_mean_deltaBG = (elev_mean_rts - elev_mean_bg),
         sr_mean_deltaBG = (sr_mean_rts - sr_mean_bg),
         r_mean_deltaWater = (r_mean_rts - r_mean_water),
         g_mean_deltaWater = (g_mean_rts - g_mean_water),
         b_mean_deltaWater = (b_mean_rts - b_mean_water),
         lum_mean_deltaWater = (lum_mean_rts - lum_mean_water),
         nir_mean_deltaWater = (nir_mean_rts - nir_mean_water),
         ndvi_mean_deltaWater = (ndvi_mean_rts - ndvi_mean_water),
         ndwi_mean_deltaWater = (ndwi_mean_rts - ndwi_mean_water),
         elev_mean_deltaWater = (elev_mean_rts - elev_mean_water),
         sr_mean_deltaWater = (sr_mean_rts - sr_mean_water),
         r_sd_deltaBG = (r_sd_rts - r_sd_bg),
         b_sd_deltaBG = (b_sd_rts - b_sd_bg),
         g_sd_deltaBG = (g_sd_rts - g_sd_bg),
         lum_sd_deltaBG = (lum_sd_rts - lum_sd_bg),
         nir_sd_deltaBG = (nir_sd_rts - nir_sd_bg),
         ndvi_sd_deltaBG = (ndvi_sd_rts - ndvi_sd_bg),
         ndwi_sd_deltaBG = (ndwi_sd_rts - ndwi_sd_bg),
         elev_sd_deltaBG = (elev_sd_rts - elev_sd_bg),
         sr_sd_deltaBG = (sr_sd_rts - sr_sd_bg),
         r_sd_deltaWater = (r_sd_rts - r_sd_water),
         b_sd_deltaWater = (b_sd_rts - b_sd_water),
         g_sd_deltaWater = (g_sd_rts - g_sd_water),
         lum_sd_deltaWater = (lum_sd_rts - lum_sd_water),
         nir_sd_deltaWater = (nir_sd_rts - nir_sd_water),
         ndvi_sd_deltaWater = (ndvi_sd_rts - ndvi_sd_water),
         ndwi_sd_deltaWater = (ndwi_sd_rts - ndwi_sd_water),
         elev_sd_deltaWater = (elev_sd_rts - elev_sd_water),
         sr_sd_deltaWater = (sr_sd_rts - sr_sd_water)) %>%
  pivot_longer(r_mean_bg:sr_sd_deltaWater, 
               names_to = 'variable', 
               values_to = 'value') %>%
  separate(variable, 
  ...
  mutate(rts_class = factor(case_when(rts_class == 'rts' ~ 'RTS',
                                      ...
                                      rts_class == 'water' ~ 'Water',
                                      rts_class == 'deltaBG' ~ 'Delta BG',
                                      rts_class == 'deltaWater' ~ 'Delta Water'),
                            levels = c('RTS', 'Background', 'Water', 'Delta BG', 'Delta Water'))) %>%
...
Summary Figure of Zonal Stats

```{r}
zonal_stats_variables <- zonal_stats_contrasts %>%
  filter(p_value <= 0.1 & 
           !(variable %in% c('lum_mean', 'lum_sd', 'elev_sd', 'nir_sd')) &
           !str_detect(rts_class, 'Water')) %>%
  pull(variable) %>%
  unique()
zonal_stats_variables <- c(zonal_stats_variables, 'b_mean')

zonal_stats_mean <- zonal_stats %>%
  filter(!str_detect(rts_class, 'Water')) %>%
  pivot_longer(r_mean:sr_sd, names_to = 'variable', values_to = 'value') %>%
  summarise(value_mean = mean(value, na.rm = TRUE),
            value_sd = sd(value, na.rm = TRUE),
            value_se = sd(value, na.rm = TRUE)/n(),
            .by = c('imagery', 'pq_ci', 'rts_class', 'variable')) %>%
  filter(variable %in% zonal_stats_variables) %>%
  mutate(variable = factor(variable,
                           levels = c('r_mean',
                                      ...
                                      'g_sd',
                                      'b_sd')),
         variable_num = case_when(
           pq_ci == 'High' ~ as.numeric(variable) - 0.2,
           pq_ci == 'Expected' ~ as.numeric(variable),
           pq_ci == 'Low' ~ as.numeric(variable) + 0.2
           ),
         rts_class = factor(
           case_when(rts_class == 'Delta BG' ~ 'Delta (RTS - BG)',
                     TRUE ~ rts_class),
           levels = c('RTS', 'Delta (RTS - BG)', 'Background'))
         )

zonal_stats_contrasts_mean <- zonal_stats_contrasts %>%
  filter(
    p_value <= 0.1 & 
      variable %in% c(zonal_stats_variables) &
      rts_class %in% c('RTS', 'Delta BG', 'Background')
  ) %>%
  mutate(
    rts_class = factor(
      case_when(rts_class == 'Delta BG' ~ 'Delta (RTS - BG)',
    ...
    xmin = case_when(
      contrast %in% c('High - Expected', 'High - Low') ~ variable_num - 0.19,
      contrast == 'Expected - Low' ~ variable_num + 0.01
    ),
    xmax = case_when(
      contrast %in% c('Expected - Low', 'High - Low') ~ variable_num + 0.19,
      contrast == 'High - Expected' ~ variable_num - 0.01
    ),
    y_adjust = case_when(p_value <= 0.05 ~ 0.6,
                         p_value > 0.05 ~ 0)
  ) %>%
  select(imagery, rts_class, variable, contrast, estimate, se, p_value, star_label, 
         y_adjust, variable_num, xmin, xmax) %>%
  left_join(zonal_stats_mean %>%
              mutate(upr_sd = value_mean + value_sd,
                     upr_se = value_mean + value_se) %>%
              filter(upr_sd == max(upr_sd),
                     .by = c('imagery', 'variable', 'rts_class')) %>%
              select(imagery, variable, rts_class, upr_sd, upr_se),
            by = c('imagery', 'variable', 'rts_class')) %>%
  mutate(
    y_sd = case_when(
      contrast %in% c('High - Expected', 'Expected - Low') ~ 
        upr_sd + 0.05,
      contrast == 'High - Low' ~ 
        upr_sd + 0.2
    ),
    y_se = case_when(
      ...
      contrast == 'High - Low' ~ 
        upr_se + 0.2
    ),group = paste(variable, contrast, sep = '_')
  )
```
...
x_axis_labels <- c('Red',
                   ...
                   'NDVI',
                   'NDWI',
                   expression(sigma ~ ' Red'),
                   expression(sigma ~ ' Green'),
                   expression(sigma ~ ' Blue'))

# with SD for error bars
ggplot(zonal_stats_mean,
       aes(x = variable_num, color = pq_ci)) +
  # geom_vline(xintercept = c(0.5, 4.5, 6.5, 7.5, 11.5, 12.5, 13.5), color = 'gray80') +
  geom_rect(data = tibble(xmin = c(0.5, 5.5),
                          xmax = c(3.5, 6.5),
                          ymin = -Inf,
                          ymax = Inf),
            aes(xmin = xmin,
  ...
  geom_errorbar(aes(ymin = value_mean - value_sd, 
                ...
                width = 0.25,
                linewidth = 0.25) +
  geom_signif(data = zonal_stats_contrasts_mean,
              aes(x = xmin, y = y_sd, xend = xmax, yend = y_sd,
                  annotation = star_label, vjust = y_adjust,
                  group = group),
              stat = "identity",
              manual = TRUE,
              tip_length = 1,
  ...
  geom_text(
    data = tibble(
      x = c(2, 4.5, 6, 8),
      y = rep(c(-1.4, -1.2), 2),
      label = c('Luminance', 'Plants', 'Moisture', 'Patchiness')
    ),
    aes(x = x, y = y, label = label),
    inherit.aes = FALSE,
    size = 2.5
  ...
  scale_x_continuous(
    name = 'Variable',
    breaks = seq(1:length(unique(zonal_stats_mean$variable))),
    labels = x_axis_labels,
    expand = expansion(mult = c(0.02, 0.033))
    ) +
...
# ggsave('./figures/zonal_stats_summary_sd.pdf',
#        height = 6,
#        width = 6.5)
# ggsave('./figures/zonal_stats_summary_sd.jpg',
#        height = 6,
#        width = 6.5)

# with SD for error bars and no delta
ggplot(zonal_stats_mean %>%
         filter(rts_class != 'Delta (RTS - BG)'),
       aes(x = variable_num, color = pq_ci)) +
  # geom_vline(xintercept = c(0.5, 4.5, 6.5, 7.5, 11.5, 12.5, 13.5), color = 'gray80') +
  geom_rect(data = tibble(xmin = c(0.5, 5.5),
                          xmax = c(3.5, 6.5),
...
# ggsave('./figures/zonal_stats_summary_sd_no_delta.pdf',
#        height = 4.25,
#        width = 6.5)
# ggsave('./figures/zonal_stats_summary_sd_no_delta.jpg',
#        height = 4.25,
#        width = 6.5)

# simplified version with only delta
ggplot(zonal_stats_mean %>%
         filter(rts_class == "Delta (RTS - BG)"),
       aes(x = variable_num, color = pq_ci)) +
  # geom_vline(xintercept = c(0.5, 4.5, 6.5, 7.5, 11.5, 12.5, 13.5), color = 'gray80') +
...
# ggsave('./figures/zonal_stats_summary_delta_sd.pdf',
#        height = 2.5,
#        width = 6.5)
# ggsave('./figures/zonal_stats_summary_delta_sd.jpg',
#        height = 2.5,
#        width = 6.5)

# with SE for error bars
ggplot(zonal_stats_mean,
       aes(x = variable_num, color = pq_ci)) +
  # geom_vline(xintercept = c(0.5, 4.5, 6.5, 7.5, 11.5, 12.5, 13.5), color = 'gray80') +
  geom_rect(data = tibble(xmin = c(0.5, 5.5),
  ...
  geom_errorbar(aes(ymin = value_mean - value_se, 
                ...
                linewidth = 0.25) +
  geom_signif(data = zonal_stats_contrasts_mean,
              aes(x = xmin, y = y_se, xend = xmax, yend = y_se,
                  annotation = star_label, vjust = y_adjust,
                  group = group),
              stat = "identity",
              manual = TRUE,
...
# ggsave('./figures/zonal_stats_summary_se.pdf',
#        height = 6,
#        width = 6.5)
# ggsave('./figures/zonal_stats_summary_se.jpg',
#        height = 6,
#        width = 6.5)
```
...
- Bright RTS are predicted better in Planet imagery.
...
lum_delta_fig <- ggplot(
  ...
  geom_signif(
    data = zonal_stats_contrasts %>%
      filter(p_value <= 0.1 & variable == 'lum_mean' & rts_class == 'Delta BG'),
    aes(x = x_min, y = y, xend = x_max, yend = y,
        annotation = star_label, group = contrast),
    stat="identity",
    manual = TRUE,
    ...
    coord_cartesian(ylim = zonal_stats_limits %>%
                      filter(variable == 'lum_mean' & rts_class == 'Delta BG') %>%
                      ungroup() %>%
                      select(lwr, upr) %>%
                      as_vector())
lum_delta_fig
...
r_delta_fig <- ggplot(
  ...
  geom_signif(
    data = zonal_stats_contrasts %>%
      filter(p_value <= 0.1 & variable == 'r_mean' & rts_class == 'Delta BG'),
    aes(x = x_min, y = y, xend = x_max, yend = y,
        annotation = star_label, group = contrast),
    stat="identity",
    manual = TRUE,
    ...
    coord_cartesian(ylim = zonal_stats_limits %>%
                      filter(variable == 'r_mean' & rts_class == 'Delta BG') %>%
                      ungroup() %>%
                      select(lwr, upr) %>%
                      as_vector())
...
g_delta_fig <- ggplot(
  ...
  geom_signif(
    data = zonal_stats_contrasts %>%
      filter(p_value <= 0.1 & variable == 'g_mean' & rts_class == 'Delta BG'),
    aes(x = x_min, y = y, xend = x_max, yend = y,
        annotation = star_label, group = contrast),
    stat="identity",
    manual = TRUE,
    ...
    coord_cartesian(ylim = zonal_stats_limits %>%
                      filter(variable == 'g_mean' & rts_class == 'Delta BG') %>%
                      ungroup() %>%
                      select(lwr, upr) %>%
                      as_vector())
...
b_delta_fig <- ggplot(
  ...
  geom_signif(
    data = zonal_stats_contrasts %>%
      filter(p_value <= 0.1 & variable == 'b_mean' & rts_class == 'Delta BG'),
    aes(x = x_min, y = y, xend = x_max, yend = y,
        annotation = star_label, group = contrast),
    stat="identity",
    manual = TRUE,
    ...
    coord_cartesian(ylim = zonal_stats_limits %>%
                      filter(variable == 'b_mean' & rts_class == 'Delta BG') %>%
                      ungroup() %>%
                      select(lwr, upr) %>%
                      as_vector())
...
- More plants within RTS or fewer plants in background pixels leads to poor predictions, as seen in NIR and NDVI

```{r, fig.height = 5, fig.width = 6.5}
zonal_stats_figs[['nir_mean']]
zonal_stats_figs[['ndvi_mean']]

zonal_stats_nir <- plot_zonal_stats_2(
  'pq_ci', 
  'nir_mean', 
  zonal_stats_contrasts, 
  zonal_stats_limits
)
zonal_stats_nir

zonal_stats_ndvi <- plot_zonal_stats_2(
  'pq_ci', 
  'ndvi_mean', 
  zonal_stats_contrasts, 
  zonal_stats_limits
...
- RTS were predicted poorly in wet landscapes (PlanetScope)

```{r, fig.height = 8, fig.width = 6.5}
zonal_stats_figs[['ndwi_mean']]

zonal_stats_ndwi <- plot_zonal_stats_2(
  'pq_ci', 
  'ndwi_mean', 
  zonal_stats_contrasts, 
  zonal_stats_limits
...
- RTS were predicted poorly where the background was patchy/highly variable in the visible bands
...
zonal_stats_rgb_sd <- plot_zonal_stats_3(
  'pq_ci',
  c('lum_sd', 'r_sd', 'g_sd', 'b_sd'),
  zonal_stats_contrasts,
  zonal_stats_limits
  )
zonal_stats_rgb_sd
...
rts_maps_subset <- map(
  id_subset,
  ~ plot_pred_rgb_2(.x, imagery_labels, map_dimensions)
)
```
...
rts_highlights_plot <- (
  ...
  rts_maps_subset[[5]] +
    facet_grid('Variable Performance' ~ .)
) / (
  rts_maps_subset[[6]] +
...
Visualize the RTS predictions on RGB imagery
...
Visualize the RTS predictions on non-RGB layers

```{r, fig.width = 12, fig.height = 4.8}
rts_maps_nir <- map(pred_ids,
                 ~ plot_pred_single_band(.x, 'nir'))
walk(rts_maps_nir,
     ~ print(.x))
...
```{r, fig.width = 12, fig.height = 4.8}
rts_maps_ndvi <- map(pred_ids,
                 ~ plot_pred_single_band(.x, 'ndvi'))
walk(rts_maps_ndvi,
     ~ print(.x))
...
```{r, fig.width = 12, fig.height = 4.8}
rts_maps_ndwi <- map(pred_ids,
                 ~ plot_pred_single_band(.x, 'ndwi'))
walk(rts_maps_ndwi,
     ~ print(.x))
```

```{r, fig.width = 12, fig.height = 4.8}
rts_maps_elev <- map(pred_ids,
                 ~ plot_pred_single_band(.x, 'elev'))
walk(rts_maps_elev,
     ~ print(.x))
...
```{r, fig.width = 12, fig.height = 4.8}
rts_maps_sr <- map(pred_ids,
                 ~ plot_pred_single_band(.x, 'sr'))
walk(rts_maps_sr,
     ~ print(.x))
...
## Planet Processing Examples

```{r}
pre_arosics <- rast('./data/planet/processing_examples/0_20180709_064535_1014_3B_AnalyticMS_SR_harmonized_clip_masked.tiff') %>%
  project('EPSG:3413')
RGB(pre_arosics) <- c(1, 2, 3)

pre_mad <- rast('./data/planet/processing_examples/0_20180709_064535_1014_3B_AnalyticMS_SR_harmonized_clip_arosics.tif') %>%
  project('EPSG:3413')
RGB(pre_mad) <- c(1, 2, 3)

post_mad <- rast('./data/planet/processing_examples/0_20180709_064535_1014_3B_AnalyticMS_SR_harmonized_clip_arosics_mad.tif') %>%
  project('EPSG:3413')
RGB(post_mad) <- c(1, 2, 3)

sentinel <- rast('./data/planet/processing_examples/Sentinel-2_YG_Validation_mosaic_bgrnir_Region_0_UTM42N.tiff') %>%
  project(pre_mad)
RGB(sentinel) <- c(1, 2, 3)
sentinel_crop <- sentinel %>%
  crop(pre_mad, mask = TRUE)
```

```{r}
pre_arosics_df <- pre_arosics %>%
  as.data.frame(xy = TRUE) %>%
  rename(r = 5, g = 4, b = 3) %>%
  full_join(pre_arosics %>%
              stretch(minv = 0,
                      maxv = 1,
                      smin = 300,
                      smax = 1200) %>%
              as.data.frame(xy = TRUE) %>%
              rename(r_lin = 5, g_lin = 4, b_lin = 3) %>%
              select(-nir),
            by = c('x', 'y')) %>%
  mutate(
    color.hex = factor(
      case_when(
        !is.na(r_lin) & !is.na(g_lin) & !is.na(b_lin) ~ rgb(r_lin, g_lin, b_lin, maxColorValue = 1),
        TRUE ~ NA
      )
    )
  )
...
```{r}
# x_limits <- c(1932000, 1942500)
# y_limits <- c(818500, 827500)
x_limits <- c(1940250, 1940550)
y_limits <- c(821500, 821800)
limits <- sentinel_df %>% 
  slice(
    map2_int(x_limits,
             y_limits, 
             ~ which(
               sqrt((sentinel_df$x - .x)^2 + (sentinel_df$y - .y)^2) == min(sqrt((sentinel_df$x - .x)^2 + (sentinel_df$y - .y)^2))
             )
    )
  ) %>% 
  select(x, y)

sentinel_df_clean <- sentinel_df %>%
  filter(between(x, min(limits$x), max(limits$x)) & 
           between(y, min(limits$y), max(limits$y)) & 
           !is.na(color.hex)) %>%
  mutate(color.hex = factor(as.character(color.hex)))

pre_arosics_df_clean <- pre_arosics_df %>%
  filter(between(x, min(limits$x), max(limits$x)) & 
           between(y, min(limits$y), max(limits$y)) & 
           !is.na(color.hex)) %>%
  mutate(color.hex = factor(as.character(color.hex)))

pre_mad_df_clean <- pre_mad_df %>%
  filter(between(x, min(limits$x), max(limits$x)) & 
           between(y, min(limits$y), max(limits$y)) & 
           !is.na(color.hex)) %>%
  mutate(color.hex = factor(as.character(color.hex)))

post_mad_df_clean <- post_mad_df %>%
  filter(between(x, min(limits$x), max(limits$x)) & 
           between(y, min(limits$y), max(limits$y)) & 
           !is.na(color.hex)) %>%
  mutate(color.hex = factor(as.character(color.hex)))
...
sentinel_map <- ggplot() +
  geom_raster(data = sentinel_df_clean,
              aes(x = x, y = y, fill = color.hex)) +
  scale_fill_manual(values = levels(sentinel_df_clean$color.hex),
                    guide = FALSE) +
...
pre_arosics_map <- ggplot() +
  # geom_raster(data = sentinel_df,
  #             aes(x = x, y = y, fill = color.hex)) +
  # scale_fill_manual(values = levels(sentinel_df$color.hex),
  #                   guide = FALSE) +
  # new_scale_fill() +
  geom_raster(data = pre_arosics_df_clean,
              aes(x = x, y = y, fill = color.hex)) +
  scale_fill_manual(values = levels(pre_arosics_df_clean$color.hex),
                    guide = FALSE) +
  scale_x_continuous(name = 'Longitude',
                     limits = limits$x) +
  scale_y_continuous(name = "Latitude",
...
pre_mad_map <- ggplot() +
  ...
  #                   guide = FALSE) +
  # new_scale_fill() +
  geom_raster(data = pre_mad_df_clean,
              aes(x = x, y = y, fill = color.hex)) +
  scale_fill_manual(values = levels(pre_mad_df_clean$color.hex),
                    guide = FALSE) +
  scale_x_continuous(name = 'Longitude',
                     limits = limits$x) +
  scale_y_continuous(name = "Latitude",
...
post_mad_map <- ggplot() +
  ...
  #                   guide = FALSE) +
  # new_scale_fill() +
  geom_raster(data = post_mad_df_clean,
              aes(x = x, y = y, fill = color.hex)) +
  scale_fill_manual(values = levels(post_mad_df_clean$color.hex),
                    guide = FALSE) +
  scale_x_continuous(name = 'Longitude',
                     limits = limits$x) +
  scale_y_continuous(name = "Latitude",
...
```{r, fig.height = 6.75, fig.width = 6.5}
planet_processing_map <- (
  (
    sentinel_map +
      theme(axis.title = element_blank(),
            axis.text = element_blank(),
  ...
  plot_annotation(tag_levels = list(c('a. Sentinel-2 (Reference)', 
                                      'b. Raw PlanetScope',  
                                      'c. PlanetScope (AROSICS)', 
                                      'd. PlanetScope (AROSICS + IRMAD)'))) &
  theme(plot.tag.position = c(0, 1),
        plot.tag = element_text(size = 12, hjust = 0, vjust = -0.4))

planet_processing_map

# ggsave('./figures/planet_processing_map.pdf',
#        planet_processing_map,
#        height = 6.75,
#        width = 6.5)
# ggsave('./figures/planet_processing_map.jpg',
#        planet_processing_map,
#        height = 6.75,
#        width = 6.5)
```
...
hist_pre <- ggplot(pre_mad_df) +
  ...
  geom_density(aes(x = b),
               ...
               fill = 'blue',
               alpha = 0.33) +
  scale_x_continuous(name = 'Surface Reflectance',
                     limits = x_limits,
                     expand = c(0, 0)) +
  scale_y_continuous(name = 'Density',
                     limits = y_limits) +
...
# ggsave('./figures/irmad_processing_histogram.pdf',
#        processing_hist,
#        height = 3,
#        width = 5)
# ggsave('./figures/irmad_processing_histogram.jpg',
#        processing_hist,
#        height = 3,
#        width = 5)
```
