...
-   Case study of RTS features that had different prediction qualities across all 3 models.
-   Can a random forest model (or some other analysis) predict prediction quality?
-   Add stats to prediction quality by region figure. Somehow include size in this analysis.
...
input_data <- input_data %>%
  left_join(polys %>%
              ...
              select(id, imagery, region, rts_area, rts_iou, mean_iou, mean_iou_v2, 
                     pq_ci, pq_ci_size, pq_ci_offset),
            by = c('id', 'imagery'))
```

```{r}
zonal_stats <- input_data %>%
  group_by(imagery, id, region, rts_class, rts_area, rts_iou, mean_iou, 
...
ggplot(polys,
       aes(x = rts_area, y = rts_iou, shape = region, color = pq_ci)) +
  geom_point() +
  scale_x_log10(name = expression('RTS Area (m'^2*')')) +
  scale_y_continuous(name = 'RTS IoU') +
...
ggplot(polys, aes(x = rts_area, y = rts_shape, color = pq_n_unique)) +
  geom_point() +
  scale_x_log10()
```
