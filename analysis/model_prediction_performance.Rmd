...
```{r, echo = FALSE}
avg_precision <- function(truth, iou, thresholds) {
  ...
  recall <- c(recall, 0)
  ap <- sum((recall[1:length(recall) - 1] - recall[2:length(recall)])*precision, na.rm = TRUE)
  return(ap)
...
maxar_map <- avg_precision(filter(polys, imagery == 'maxar')$ground_truth, 
                           filter(polys, imagery == 'maxar')$iou, 
                           thresholds)

planet_map <- avg_precision(filter(polys, imagery == 'planet')$ground_truth, 
                           filter(polys, imagery == 'planet')$iou, 
                           thresholds)

sentinel_map <- avg_precision(filter(polys, imagery == 'sentinel')$ground_truth, 
...
ggplot(polys, 
  ...
  geom_point(position = position_jitterdodge(seed = 1, dodge.width = 0.9, jitter.width = 0.5),
             aes(color = imagery),
             size = 1,
             alpha = 0.1) +
