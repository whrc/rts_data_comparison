...
-   Calculate metric of feature shape and incorporate into analysis!
-   Density plots showing prediction quality somehow. I've done a few, and I don't think this is going to help at all.
-   some sort of figure and/or decision tree for how other researchers can choose what resolution/imagery quality is necessary for their analysis.
-   Start picking figures to include in manuscript!
-   Try comparing performance with RTS IoUs
-   Histogram of rts_iou scores for each imagery type. Larger SD is less desirable.
...
```{r}
lm_contrasts_pq_ci <- function(model) {
...
### lm_contrasts_pq_ci_size

```{r}
lm_contrasts_pq_ci_size <- function(model) {
  contrasts <- as_tibble(emmeans(model$fit,
                                 pairwise ~ pq_ci_size,
                                 adjust = 'tukey')$contrasts) %>%
    mutate(star_label = assign_conf_stars(p.value)) %>%
...
```{r}
nls_boot_predict <- function(boot_ci, newdata, lwr, upr) {
  
  coefboot <- boot_ci[['coefboot']]
  
  output <- tibble()
  for (new_value in as.numeric(newdata$rts_area)) {
    
    predictions <- c()
    for (row_idx in 1:nrow(coefboot)) {
      coefs <- coefboot[row_idx,]
      predictions <- c(predictions, coefs[['Vmax']] * new_value / (coefs[['Km']] + new_value))
    }
    
    prediction <- quantile(predictions, probs = c(0.025, 0.5, 0.975))
    prediction_df <- tibble(rts_area = new_value,
                            lwr = prediction[1],
                            median = prediction[2],
                            upr = prediction[3])
    output <- rbind(output,
...
polys <- polys %>%
  full_join(val_polys %>%
              as_tibble(),
            by = c('id', 'imagery')) %>%
  mutate(rts_shape = as.numeric(4*pi*st_area(val_geometry)/st_perimeter(val_geometry)^2)) # calculate Polsby-Popper value (shape compactness/roundness)
...
```{r}
polys <- polys %>%
  mutate(tile_rts_area = round(as.numeric(st_area(val_geometry)), 2),
...
# maxar_rts_iou_nls <- nls(rts_iou ~ Vmax * rts_area / (Km + rts_area),
#                start = list(Km = 50, Vmax = 2),
#                data = polys %>%
#                  filter(imagery == 'Maxar'))
# 
# maxar_rts_iou_boot <- nlsBoot(maxar_rts_iou_nls, 500000)
# saveRDS(maxar_rts_iou_nls, './data/models/maxar_rts_iou_nls.RData')
# saveRDS(maxar_rts_iou_boot, './data/models/maxar_rts_iou_boot.RData')

maxar_rts_iou_nls <- readRDS('./data/models/maxar_rts_iou_nls.RData')
maxar_rts_iou_boot <- readRDS('./data/models/maxar_rts_iou_boot.RData')
```
...
# planet_rts_iou_nls <- nls(rts_iou ~ Vmax * rts_area / (Km + rts_area),
#                start = list(Km = 50, Vmax = 2),
#                data = polys %>%
#                  filter(imagery == 'Planet'))
# 
# planet_rts_iou_boot <- nlsBoot(planet_rts_iou_nls, 500000)
# saveRDS(planet_rts_iou_nls, './data/models/planet_rts_iou_nls.RData')
# saveRDS(planet_rts_iou_boot, './data/models/planet_rts_iou_boot.RData')

planet_rts_iou_nls <- readRDS('./data/models/planet_rts_iou_nls.RData')
planet_rts_iou_boot <- readRDS('./data/models/planet_rts_iou_boot.RData')
```
...
# sentinel_rts_iou_nls <- nls(rts_iou ~ Vmax * rts_area / (Km + rts_area),
#                start = list(Km = 50, Vmax = 2),
#                data = polys %>%
#                  filter(imagery == 'Sentinel-2'))
# 
# sentinel_rts_iou_boot <- nlsBoot(sentinel_rts_iou_nls, 500000)
# saveRDS(sentinel_rts_iou_nls, './data/models/sentinel_rts_iou_nls.RData')
# saveRDS(sentinel_rts_iou_boot, './data/models/sentinel_rts_iou_boot.RData')

sentinel_rts_iou_nls <- readRDS('./data/models/sentinel_rts_iou_nls.RData')
sentinel_rts_iou_boot <- readRDS('./data/models/sentinel_rts_iou_boot.RData')
```
...
```{r}
test <- nls_boot_predict(maxar_iou_boot, newdata, lwr = 0.025, upr = 0.975)

maxar_iou_ci <- newdata %>%
  mutate(predict = predict(maxar_iou_nls, newdata),
         predict_lwr = maxar_iou_boot[["bootCI"]][2, 2] * rts_area / 
           (maxar_iou_boot[["bootCI"]][1, 3] + rts_area),
         predict_upr = maxar_iou_boot[["bootCI"]][2, 3] * rts_area / 
           (maxar_iou_boot[["bootCI"]][1, 2] + rts_area))

maxar_rts_iou_ci <- newdata %>%
  mutate(predict = predict(maxar_rts_iou_nls, newdata),
         predict_lwr = maxar_rts_iou_boot[["bootCI"]][2, 2] * rts_area / 
           (maxar_rts_iou_boot[["bootCI"]][1, 3] + rts_area),
         predict_upr = maxar_rts_iou_boot[["bootCI"]][2, 3] * rts_area / 
           (maxar_rts_iou_boot[["bootCI"]][1, 2] + rts_area))
```

```{r}
planet_iou_ci <- newdata %>%
  mutate(predict = predict(planet_iou_nls, newdata),
         predict_lwr = planet_iou_boot[["bootCI"]][2, 2] * rts_area / 
           (planet_iou_boot[["bootCI"]][1, 3] + rts_area),
         predict_upr = planet_iou_boot[["bootCI"]][2, 3] * rts_area / 
           (planet_iou_boot[["bootCI"]][1, 2] + rts_area))

planet_rts_iou_ci <- newdata %>%
  mutate(predict = predict(planet_rts_iou_nls, newdata),
         predict_lwr = planet_rts_iou_boot[["bootCI"]][2, 2] * rts_area / 
           (planet_rts_iou_boot[["bootCI"]][1, 3] + rts_area),
         predict_upr = planet_rts_iou_boot[["bootCI"]][2, 3] * rts_area / 
           (planet_rts_iou_boot[["bootCI"]][1, 2] + rts_area))
```

```{r}
sentinel_iou_ci <- newdata %>%
  mutate(predict = predict(sentinel_iou_nls, newdata),
         predict_lwr = sentinel_iou_boot[["bootCI"]][2, 2] * rts_area / 
           (sentinel_iou_boot[["bootCI"]][1, 3] + rts_area),
         predict_upr = sentinel_iou_boot[["bootCI"]][2, 3] * rts_area / 
           (sentinel_iou_boot[["bootCI"]][1, 2] + rts_area))

sentinel_rts_iou_ci <- newdata %>%
  mutate(predict = predict(sentinel_rts_iou_nls, newdata),
         predict_lwr = sentinel_rts_iou_boot[["bootCI"]][2, 2] * rts_area / 
           (sentinel_rts_iou_boot[["bootCI"]][1, 3] + rts_area),
         predict_upr = sentinel_rts_iou_boot[["bootCI"]][2, 3] * rts_area / 
           (sentinel_rts_iou_boot[["bootCI"]][1, 2] + rts_area))
```

```{r}
# Add predictions to `polys`
mean_iou_predictions <- c(predict(maxar_iou_nls, 
...
rts_iou_predictions <- c(predict(maxar_iou_nls, 
                                 filter(polys, imagery == 'Maxar')),
                         predict(planet_iou_nls, 
                                 filter(polys, imagery == 'Planet')),
                         predict(sentinel_iou_nls, 
...
polys <- polys %>%
  mutate(
    mean_iou_pred = mean_iou_predictions,
    mean_iou_pred_lwr = case_when(
      imagery == 'Maxar' ~ 
        maxar_iou_boot[["bootCI"]][2, 2] * rts_area / 
        (maxar_iou_boot[["bootCI"]][1, 3] + rts_area),
      imagery == 'Planet' ~ 
        planet_iou_boot[["bootCI"]][2, 2] * rts_area / 
        (planet_iou_boot[["bootCI"]][1, 3] + rts_area),
      imagery == 'Sentinel-2' ~ 
        sentinel_iou_boot[["bootCI"]][2, 2] * rts_area / 
        (sentinel_iou_boot[["bootCI"]][1, 3] + rts_area)
    ),
    mean_iou_pred_upr = case_when(
      imagery == 'Maxar' ~ 
        maxar_iou_boot[["bootCI"]][2, 3] * rts_area / 
        (maxar_iou_boot[["bootCI"]][1, 2] + rts_area),
      imagery == 'Planet' ~ 
        planet_iou_boot[["bootCI"]][2, 3] * rts_area / 
        (planet_iou_boot[["bootCI"]][1, 2] + rts_area),
      imagery == 'Sentinel-2' ~ 
        sentinel_iou_boot[["bootCI"]][2, 3] * rts_area / 
        (sentinel_iou_boot[["bootCI"]][1, 2] + rts_area)
    ),
      rts_iou_pred = rts_iou_predictions,
    rts_iou_pred_lwr = case_when(
      imagery == 'Maxar' ~ 
        maxar_rts_iou_boot[["bootCI"]][2, 2] * rts_area / 
        (maxar_rts_iou_boot[["bootCI"]][1, 3] + rts_area),
      imagery == 'Planet' ~ 
        planet_rts_iou_boot[["bootCI"]][2, 2] * rts_area / 
        (planet_rts_iou_boot[["bootCI"]][1, 3] + rts_area),
      imagery == 'Sentinel-2' ~ 
        sentinel_rts_iou_boot[["bootCI"]][2, 2] * rts_area / 
        (sentinel_rts_iou_boot[["bootCI"]][1, 3] + rts_area)
    ),
    rts_iou_pred_upr = case_when(
      imagery == 'Maxar' ~ 
        maxar_rts_iou_boot[["bootCI"]][2, 3] * rts_area / 
        (maxar_rts_iou_boot[["bootCI"]][1, 2] + rts_area),
      imagery == 'Planet' ~ 
        planet_rts_iou_boot[["bootCI"]][2, 3] * rts_area / 
        (planet_rts_iou_boot[["bootCI"]][1, 2] + rts_area),
      imagery == 'Sentinel-2' ~ 
        sentinel_rts_iou_boot[["bootCI"]][2, 3] * rts_area / 
        (sentinel_rts_iou_boot[["bootCI"]][1, 2] + rts_area)
    )
  )
```
...
rts_iou_ci <- maxar_rts_iou_ci %>%
  mutate(imagery = 'Maxar') %>%
  rbind(planet_rts_iou_ci %>%
          mutate(imagery = 'Planet')) %>%
  rbind(sentinel_rts_iou_ci %>%
          mutate(imagery = 'Sentinel-2'))

rm(maxar_iou_nls, maxar_iou_ci, maxar_rts_iou_nls, maxar_rts_iou_ci,
   planet_iou_nls, planet_iou_ci, planet_rts_iou_nls, planet_rts_iou_ci,
   sentinel_iou_nls, sentinel_iou_ci, sentinel_rts_iou_nls, sentinel_rts_iou_ci,
...
ggplot(polys, aes(x = rts_area, y = rts_iou, color = imagery)) +
  geom_point(size = 1) +
  geom_ribbon(data = rts_iou_ci,
            aes(x = rts_area, ymin = predict_lwr, ymax = predict_upr, group = imagery),
            alpha = 0.2,
            inherit.aes = FALSE) +
  geom_line(data = rts_iou_ci, 
            aes(x = rts_area, y = predict, color = imagery),
            inherit.aes = FALSE) +
  scale_x_continuous(name = expression('RTS Area (m'^2*')')) +
...
maxar_d1 <- D(expression(y = 0.7254422 * x / (2123.6825320 + x)), name = 'x')
planet_d1 <- D(expression(y = 0.6906648 * x / (2498.0072184 + x)), name = 'x')
sentinel_d1 <- D(expression(y = 0.5895414 * x / (2885.9723242 + x)), name = 'x')

# estimate where the derivative crosses 1e-05
estimate_grid <- tibble(rts_area = seq(10000, 40000, by = 0.1))
...
# plot to confirm
ggplot(rts_iou_ci, aes(x = rts_area)) +
...
rts_iou_thresholds <- polys %>%
  ...
  summarise(rts_iou_mean = mean(rts_iou),
            rts_sd = sd(rts_iou)) %>%
  mutate(rts_iou_threshold = rts_iou_mean + rts_sd)
...
polys <- polys %>%
  mutate(
    ...
    pq_ci = factor(
      case_when(
        rts_iou > rts_iou_pred_upr & rts_iou > iou_threshold ~ 'High',
        rts_iou < rts_iou_pred_lwr ~ 'Low',
        TRUE ~ 'Expected'
      ),
      levels = c('High', 'Expected', 'Low')),
    pq_ci_size = factor(
      case_when(
        rts_iou > rts_iou_pred_upr & rts_iou > iou_threshold & 
          rts_area <= size_threshold ~ 'High',
        rts_iou < rts_iou_pred_lwr & rts_area > size_threshold ~ 'Low',
        TRUE ~ 'Expected'
      ),
      levels = c('High', 'Expected', 'Low'))
  )
...
```{r}
ggplot(polys, 
       aes(x = id, y = pq_ci_size_offset, color = imagery)) +
  geom_point() +
...
ggplot(rts_pq %>%
         filter(n != 0), 
       aes(x = id, y = pq_ci_size, color = pq_ci_size, size = n)) +
  geom_point() +
  scale_y_discrete(name = 'Prediction Quality') +
  scale_x_discrete(
    name = 'RTS Features',
    limits = rev
    ) +
...
input_data <- input_data %>%
  left_join(polys %>%
              as_tibble() %>%
              select(id, imagery, region, rts_area, mean_iou, mean_iou_v2, 
                     pq_ci, pq_ci_size, pq_ci_size_offset),
            by = c('id', 'imagery'))

zonal_stats <- input_data %>%
  group_by(imagery, id, region, rts_class, rts_area, mean_iou, mean_iou_v2, 
           pq_ci, pq_ci_size, pq_ci_size_offset) %>%
...
lm_output <- zonal_stats %>%
  ...
  nest() %>% 
  mutate(models_pq_ci = map(data, 
                            possibly(lm_pq_ci, otherwise = NA)),
         summary_pq_ci = map(models_pq_ci, 
                             possibly(lm_summary, otherwise = NA)),
         contrasts_pq_ci = map(models_pq_ci, 
                               possibly(lm_contrasts_pq_ci, 
                                        otherwise = NA)),
         models_pq_ci_size = map(data, 
                                 possibly(lm_pq_ci_size, otherwise = NA)),
         summary_pq_ci_size = map(models_pq_ci_size, 
                                  possibly(lm_summary, otherwise = NA)),
         contrasts_pq_ci_size = map(models_pq_ci_size, 
                                    possibly(lm_contrasts_pq_ci_size, 
                                             otherwise = NA)))
```

```{r}
zonal_stats_contrasts_pq_ci <- lm_output %>%
  select(imagery, rts_class, variable, contrasts_pq_ci) %>%
  unnest(cols = c(contrasts_pq_ci)) %>%
  filter(p_value <= 0.1) %>%
...
zonal_stats_limits_pq_ci <- zonal_stats %>%
...
```{r}
zonal_stats_contrasts_pq_ci <- zonal_stats_contrasts_pq_ci %>%
  left_join(zonal_stats_limits_pq_ci, by = c('variable', 'rts_class')) %>%
...
zonal_stats_limits_pq_ci <- zonal_stats_limits_pq_ci %>%
  left_join(zonal_stats_contrasts_pq_ci %>%
...
```{r}
zonal_stats_contrasts_pq_ci_size <- lm_output %>%
  select(imagery, rts_class, variable, contrasts_pq_ci_size) %>%
  unnest(cols = c(contrasts_pq_ci_size)) %>%
  filter(p_value <= 0.1) %>%
  separate_wider_delim(contrast, 
                       delim = ' - ', 
                       names = c('x_min', 'x_max'), 
...
# calculate figure y-limits by hand
# (1.5*IQR is the upper and lower whisker limit; 1.05 is the default axis expansion by ggplot)
zonal_stats_limits_pq_ci_size <- zonal_stats %>%
...
```{r}
zonal_stats_contrasts_pq_ci_size <- zonal_stats_contrasts_pq_ci_size %>%
  left_join(zonal_stats_limits_pq_ci_size, by = c('variable', 'rts_class')) %>%
  group_by(imagery, rts_class, variable) %>%
  mutate(n = n(),
         y = case_when(n >= 2 & contrast == 'High - Low' ~ lwr + (upr - lwr)*1.15,
                       TRUE ~ upr),
...
# add new upr limits back to limits tibble
zonal_stats_limits_pq_ci_size <- zonal_stats_limits_pq_ci_size %>%
  left_join(zonal_stats_contrasts_pq_ci_size %>%
              select(imagery, rts_class, variable, upr) %>%
              group_by(rts_class, variable) %>%
              summarise(new_upr = max(upr)),
            by = c('rts_class', 'variable')) %>%
...
variables <- colnames(zonal_stats %>%
                        ungroup() %>%
                        select(r_mean:sr_sd))
zonal_stats_figs_pq_ci <- map(
  variables,
  ~ plot_zonal_stats(
    x_var = 'pq_ci',
    y_var = .x, 
    contrasts = zonal_stats_contrasts_pq_ci,
    limits = zonal_stats_limits_pq_ci)
)
names(zonal_stats_figs_pq_ci) <- variables

zonal_stats_figs_pq_ci_size <- map(
  variables,
  ~ plot_zonal_stats(
    x = 'pq_ci_size',
    y = .x, 
    contrasts = zonal_stats_contrasts_pq_ci_size,
    limits = zonal_stats_limits_pq_ci_size)
)
names(zonal_stats_figs_pq_ci_size) <- variables
```

A few takeaway points:

-   NIR and NDVI (mean) figures indicate that RTS are predicted poorly where background cells have low NDVI/plant growth (plants reflect NIR, so have a higher NIR value). I.e. it's harder for the model to find the RTS where there's not much plant growth across the landscape. Also important to note is that NDVI is coming from Sentinel-2, so is at lower resolution than than the RGB imagery. This could be introducting error that might explain the unexpected somewhat higher NDVI in well predicted small RTS features.

```{r, fig.width = 7, fig.height = 6}
zonal_stats_figs_pq_ci[['nir_mean']]
zonal_stats_figs_pq_ci_size[['nir_mean']]
zonal_stats_figs_pq_ci[['ndvi_mean']]
zonal_stats_figs_pq_ci_size[['ndvi_mean']]
```

-   NDWI (mean) figure indicates that RTS features are predicted better where it's drier (fewer lakes?).

```{r, fig.width = 7, fig.height = 6}
zonal_stats_figs_pq_ci[['ndwi_mean']]
zonal_stats_figs_pq_ci_size[['ndwi_mean']]
```
...
```{r, fig.width = 7, fig.height = 6}
zonal_stats_figs_pq_ci[['elev_sd']]
zonal_stats_figs_pq_ci_size[['elev_sd']]
```

I didn't find much interesting about the rest of these figures, but they are included  here for completeness:

```{r, fig.width = 7, fig.height = 6}
zonal_stats_figs_pq_ci[['r_mean']]
zonal_stats_figs_pq_ci[['g_mean']]
zonal_stats_figs_pq_ci[['b_mean']]
zonal_stats_figs_pq_ci[['lum_mean']]
zonal_stats_figs_pq_ci[['elev_mean']]
zonal_stats_figs_pq_ci[['sr_mean']]
zonal_stats_figs_pq_ci[['r_sd']]
zonal_stats_figs_pq_ci[['g_sd']]
zonal_stats_figs_pq_ci[['b_sd']]
zonal_stats_figs_pq_ci[['lum_sd']]
zonal_stats_figs_pq_ci[['nir_sd']]
zonal_stats_figs_pq_ci[['ndvi_sd']]
zonal_stats_figs_pq_ci[['ndwi_sd']]
zonal_stats_figs_pq_ci[['sr_sd']]
zonal_stats_figs_pq_ci_size[['r_mean']]
zonal_stats_figs_pq_ci_size[['g_mean']]
zonal_stats_figs_pq_ci_size[['b_mean']]
zonal_stats_figs_pq_ci_size[['lum_mean']]
zonal_stats_figs_pq_ci_size[['elev_mean']]
zonal_stats_figs_pq_ci_size[['sr_mean']]
zonal_stats_figs_pq_ci_size[['r_sd']]
zonal_stats_figs_pq_ci_size[['g_sd']]
zonal_stats_figs_pq_ci_size[['b_sd']]
zonal_stats_figs_pq_ci_size[['lum_sd']]
zonal_stats_figs_pq_ci_size[['nir_sd']]
zonal_stats_figs_pq_ci_size[['ndvi_sd']]
zonal_stats_figs_pq_ci_size[['ndwi_sd']]
zonal_stats_figs_pq_ci_size[['sr_sd']]
```
...
ggplot(input_data %>%
  ...
  scale_y_continuous(name = 'Prediction') +
  geom_hline(yintercept = 0.5, linetype = 'dashed') +
  facet_grid(rts_class ~ pq_ci_size) +
  theme_bw() +
  ggtitle('Maxar')
...
Prediction Quality (without size)
...
```{r}
zonal_stats_class %>%
  group_by(imagery, id, region) %>%
  summarise(ndwi_mean = mean(ndwi_mean),
            rts_area = first(rts_area),
            region = first(region)) %>%
ggplot(aes(x = ndwi_mean, y = rts_area, color = region)) +
  geom_point()
```
