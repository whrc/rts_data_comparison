...
```{r}
crop_poly <- tibble(lon = c(-180,180,180,-180,-180),
                    lat = c(90,90,55,55,90)) %>%
...
crop_line <- tibble(lon = c(-180, 180),
                    lat = c(55, 55)) %>%
  st_as_sf(coords = c("lon","lat"),
           crs = "EPSG:4326") %>%
  summarise(geometry = st_combine(geometry)) %>%
...
world_north <- world %>%
  ...
  st_union(by_feature = TRUE)
  # mutate(geometry = st_as_sfc(map(geom, 
  #                                 ~ st_combine(.x)),
  #                             crs = st_crs(world)))
