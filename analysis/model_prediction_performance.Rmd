...
-   Figure out why there appear to be non-zero IoU scores for predictions that didn't find any RTS.
-   Possible to remove extra influence of multiple RTS within one tile?
-   Figure out why my IoU scores don't quite match Yili's - Check removing non-existant predictions.
...
```{r, include = FALSE}
maxar_pred_dir_old <- 'MAXAR20230621_022422_p21062023'
maxar_pred_filepath_old <- paste0(loc_dir, '/maxar/predictions-', maxar_pred_dir_old)
maxar_pred_files_old <- list.files(maxar_pred_filepath_old,
                                 pattern = 'tif$',
                                 full.names = TRUE)
  
maxar_preds <- map(maxar_pred_files, ~ import_pred(.x))
maxar_preds_old <- map(maxar_pred_files_old, ~ import_pred(.x))
...
maxar_pred_polys_old <- maxar_preds_old %>%
  map_dfr(~ pred_as_poly(.x)) %>%
  mutate(imagery = 'Maxar')
...
iou <- polys %>%
  ...
  select(Imagery = imagery, `Val IoU` = mean_iou, `Val IoU Weighted` = mean_iou_weighted, `Val IoU 2` = mean_iou_v2)
...
planet_model.load_weights(model_path)
...
```{r}
maxar_input_array <- map(maxar_input, ~ as.array(.x)[,,c(1:3, 5:9)])
```

```{r}
test <- as.array(maxar_input_array)
```

```{python}
test = r.test["00000000000000000050"]
print(test.shape)
```
...
```{python}
maxar_model.input_shape
```

```{python}
# define a masker that is used to mask out partitions of the input image, this one uses a blurred background
masker_test = shap.maskers.Image("inpaint_telea", test.shape)

# By default the Partition explainer is used for all  partition explainer
explainer_test = shap.Explainer(maxar_model, masker_test)

# here we use 500 evaluations of the underlying model to estimate the SHAP values
shap_values_test = explainer(test, max_evals=500, batch_size=50, outputs=shap.Explanation.argsort.flip[:1])
