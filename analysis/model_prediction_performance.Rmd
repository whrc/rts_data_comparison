...
plot_zonal_stats <- function(x_var, y_var, contrasts, limits) {
  ...
  delta_fig <- ggplot(zonal_stats %>%
    ...
    geom_signif(data = contrasts %>%
                  filter(p_value <= 0.1 & variable == y_var & rts_class == 'Delta'),
                aes(x = x_min, y = y, xend = x_max, yend = y,
                    annotation = star_label, group = contrast),
                stat="identity",
                manual = TRUE,
...
### plot_zonal_stats_water

```{r, warning = FALSE}
plot_zonal_stats_water <- function(x_var, y_var, contrasts, limits) {
  ...
  rts_fig <- ggplot(zonal_stats_water %>%
                      filter(rts_class == 'RTS'), 
                    aes(x = .data[[x_var]], 
                        y = .data[[y_var]], 
                        color = .data[[x_var]])) +
  ...
  delta_bg_fig <- ggplot(zonal_stats_water %>%
  ...
  bg_fig <- ggplot(zonal_stats_water %>%
                     filter(rts_class == 'Background'), 
                   aes(x = .data[[x_var]], 
                       y = .data[[y_var]], 
                       color = .data[[x_var]])) +
    ...
    water_fig <- ggplot(zonal_stats_water %>%
  ...
  delta_water_fig <- ggplot(zonal_stats_water %>%
...
### plot_zonal_stats_water_2

```{r, warning = FALSE}
plot_zonal_stats_water_2 <- function(x_var, y_var, contrasts, limits) {
  ...
  delta_fig <- ggplot(zonal_stats_water %>%
                        filter(rts_class == 'Delta BG'), 
                      aes(x = .data[[x_var]], 
                          y = .data[[y_var]], 
                          color = .data[[x_var]])) +
...
### plot_zonal_stats_water_3

```{r, warning = FALSE}
plot_zonal_stats_water_3 <- function(x_var, y_vars, contrasts, limits) {
...
lm_output_water <- zonal_stats_water %>%
  select(imagery:sr_sd, pq_ci, pq_ci_size) %>%
  pivot_longer(r_mean:sr_sd, names_to = 'variable', values_to = 'value') %>%
  group_by(rts_class, imagery, variable) %>%
  nest() %>% 
...
zonal_stats_contrasts_water <- lm_output_water %>%
  select(imagery, rts_class, variable, contrasts_pq) %>%
  unnest(cols = c(contrasts_pq)) %>%
  ungroup() %>%
  separate_wider_delim(contrast, 
...
zonal_stats_limits_water <- zonal_stats_water %>%
  select(imagery:sr_sd, pq_ci) %>%
  pivot_longer(r_mean:sr_sd,  names_to = 'variable', values_to = 'value') %>%
  summarise(
    # lwr = min(trim_outliers(value), na.rm = TRUE), # use this if removing outliers from plots
...
zonal_stats_contrasts_water <- zonal_stats_contrasts_water %>%
  left_join(zonal_stats_limits_water, by = c('variable', 'rts_class')) %>%
  mutate(n = n(),
         y = case_when(
           n >= 2 & contrast == 'High - Low' ~ lwr + (upr - lwr)*1.2,
           TRUE ~ lwr + (upr - lwr)*1.05
...
# add new upr limits back to limits tibble
zonal_stats_limits_water <- zonal_stats_limits_water %>%
  left_join(zonal_stats_contrasts_water %>%
              select(imagery, rts_class, variable, upr) %>%
              group_by(rts_class, variable) %>%
              summarise(new_upr = max(upr)),
            by = c('rts_class', 'variable')) %>%
...
```{r, include = FALSE}
zonal_stats_water_figs <- map(
  variables,
  ~ plot_zonal_stats_water(
    x_var = 'pq_ci',
    y_var = .x, 
    contrasts = zonal_stats_contrasts_water %>%
      filter(p_value <= 0.1),
    limits = zonal_stats_limits_water)
)
names(zonal_stats_water_figs) <- variables
```

- Bright RTS are predicted better in Planet imagery.
```{r, fig.height = 8, fig.width = 6.5}
zonal_stats_water_figs[['lum_mean']]
zonal_stats_water_figs[['r_mean']]
zonal_stats_water_figs[['g_mean']]
zonal_stats_water_figs[['b_mean']]
...
lum_delta_fig <- ggplot(
  ...
  geom_signif(
    data = zonal_stats_contrasts_water %>%
    ...
    scale_color_manual(values = pq_colors,
                       guide = 'none') +
    facet_grid(. ~ imagery) +
    coord_cartesian(ylim = zonal_stats_limits_water %>%
...
zonal_stats_water_figs[['nir_mean']]
zonal_stats_water_figs[['ndvi_mean']]

zonal_stats_nir <- plot_zonal_stats_water_2(
  'pq_ci', 
  'nir_mean', 
  zonal_stats_contrasts_water, 
  zonal_stats_limits_water
)
zonal_stats_nir

zonal_stats_ndvi <- plot_zonal_stats_water_2(
  'pq_ci', 
  'ndvi_mean', 
  zonal_stats_contrasts_water, 
  zonal_stats_limits_water
...
```{r, fig.height = 8, fig.width = 6.5}
zonal_stats_water_figs[['ndwi_mean']]

zonal_stats_ndwi <- plot_zonal_stats_water_2(
  'pq_ci', 
  'ndwi_mean', 
  zonal_stats_contrasts_water, 
  zonal_stats_limits_water
...
```{r, fig.height = 8, fig.width = 6.5}
zonal_stats_water_figs[['lum_sd']]
zonal_stats_water_figs[['r_sd']]
zonal_stats_water_figs[['g_sd']]
zonal_stats_water_figs[['b_sd']]

zonal_stats_rgb_sd <- plot_zonal_stats_water_3(
  'pq_ci',
