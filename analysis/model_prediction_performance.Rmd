...
ggplot(polys,
  ...
  geom_smooth(method = 'lm', color = 'black') +
  scale_x_continuous(name = 'RTS Area (vector ground truth)') +
  scale_y_continuous(name = 'Prediction Area (raster prediction)') +
  facet_grid(. ~ imagery) +
  coord_fixed()
...
ggplot(polys,
  ...
  geom_point() +
  geom_smooth(method = 'lm', color = 'black') +
  scale_x_continuous(name = 'Validation Area (raster ground truth)') +
  scale_y_continuous(name = 'Prediction Area (raster prediction)') +
  facet_grid(. ~ imagery) +
  coord_fixed()

# Shape
prediction_shape <- ggplot(polys,
       aes(x = rts_shape, 
           y = pred_shape_corr)) + # use lm to correct for expected shape difference due to data type
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed') +
  geom_point() +
  geom_smooth(method = 'lm', color = 'black') +
  scale_x_continuous(name = 'RTS Shape',
                     limits = c(0, 1)) +
...
# ggsave('./figures/prediction_shape.pdf',
#        prediction_shape,
#        height = 3,
#        width = 6.5)
# ggsave('./figures/prediction_shape.jpg',
#        prediction_shape,
#        height = 3,
#        width = 6.5)

ground_pred_shape_lm_1 <- lm(
  pred_shape_corr ~ rts_shape * imagery,
  data = polys
)

summary(ground_pred_shape_lm_1)
drop1(ground_pred_shape_lm_1)
emtrends(ground_pred_shape_lm_1,
        ~ imagery,
        var = 'rts_shape')
emtrends(ground_pred_shape_lm_1,
        ~ imagery,
...
ggplot(polys,
  ...
  geom_point() +
  geom_smooth(method = 'lm', color = 'black') +
  scale_x_continuous(name = 'Validation Shape (raster ground truth)',
                     limits = c(0, 1)) +
  scale_y_continuous(name = 'Prediction Shape (raster prediction)',
                     limits = c(0, 1)) +
  facet_grid(. ~ imagery) +
  coord_fixed(expand = FALSE) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

ground_pred_shape_lm_2 <- lm(pred_shape ~ val_shape * imagery,
                             data = polys)
summary(ground_pred_shape_lm_2)
drop1(ground_pred_shape_lm_2)
emtrends(ground_pred_shape_lm_2,
        ~ imagery,
        var = 'val_shape')
emtrends(ground_pred_shape_lm_2,
        ~ imagery,
