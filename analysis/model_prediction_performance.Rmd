...
size_thresholds <- iou_ci %>%
  ...
  left_join(size_thresholds %>%
              mutate(convergence_threshold = ceiling(convergence_threshold/100)*100),
            by = 'imagery') %>%
  select(-rts_area) %>%
  left_join(iou_ci %>%
  ...
  select(`Imagery` = imagery,
         `Detection Threshold` = rts_area_exceed_0.5, 
...
convergence_threshold_y = iou_ci %>%
  filter(
    imagery == 'WorldView' & 
      abs(rts_area - segments$`Model Convergence Threshold`) == min(abs(rts_area - segments$`Model Convergence Threshold`))
                 ) %>%
  pull(prediction)
...
ggplot(iou_ci %>%
  ...
  geom_text(
    data = tibble(
      x = threshold_examples$`Detection Threshold`/2,
      y = 0.5,
      label = 'IoU = 0.5'
    ),
    aes(x = x, 
  ...
  geom_segment(x = threshold_examples$`Detection Threshold`,
               xend = threshold_examples$`Detection Threshold`,
               y = 0.5,
               yend = 0,
               color = 'black',
  ...
  geom_segment(x = threshold_examples$`Model Convergence Threshold` - 5000,
               xend = threshold_examples$`Model Convergence Threshold` + 5000,
               y = convergence_threshold_y - (1e-05*5000),
               yend = convergence_threshold_y + (1e-05*5000),
               color = 'black',
               linewidth = 0.1,
               linetype = 'dashed') +
  geom_text(
  ...
  geom_segment(x = threshold_examples$`Model Convergence Threshold`,
  ...
  scale_x_continuous(name = 'RTS Area',
                     limits = c(0, 50000)) +
