...
```{r, echo = FALSE}
mean_avg_precision <- function(df) {
  
  map <- sum((df$recall[1:nrow(df) - 1] - df$recall[2:nrow(df)])*df$precision[1:nrow(df) - 1], na.rm = TRUE)
  return(map)
...
polys <- polys %>%
  mutate(rts_iou = as.numeric(st_area(rts_int_geometry)/st_area(rts_uni_geometry)),
         ...
         mean_iou = round((rts_iou + bg_iou)/2, 2))
```
...
```{r, echo = FALSE}
# test if st_area is giving different results than rasters
calc_intersection <- function(x, y) {
  new_lyr <- x + y
  new_lyr <- classify(new_lyr,
                      matrix(c(0, NA,
                               1, NA,
                               2, 1),
                             ncol = 2,
...
custom_int_uni <- function(x) {
  id <- first(str_split(last(str_split(sources(x), 
                                       '/')[[1]]), 
                        '\\.')[[1]])
  # print(id)
  ...
  x[['rts']] <- x[['prediction']]
  x[['rts']][x[['prediction']] >= 0.5] <- 1
  x[['rts']][x[['prediction']] < 0.5] <- 0
  
  x[['bg']] <- x[['prediction']]
  x[['bg']][x[['prediction']] >= 0.5] <- 0
  x[['bg']][x[['prediction']] < 0.5] <- 1
  
  x[['val_bg']] <- classify(x[['validation']],
                            matrix(c(0, 1,
                                     1, 0),
                                   ncol = 2,
                                   byrow = TRUE))
  
  x[['int_rts']] <- calc_intersection(x[['rts']], x[['validation']])
  x[['uni_rts']] <- calc_union(x[['rts']], x[['validation']])
  x[['int_bg']] <- calc_intersection(x[['bg']], x[['val_bg']])
  x[['uni_bg']] <- calc_union(x[['bg']], x[['val_bg']])
 
   
  int_rts <- zonal(x[['sizes']], 
                   x[['int_rts']], 
                   sum) %>%
    select(int_rts = sizes)
  if (nrow(int_rts) == 0) {
    int_rts <- data.frame(int_rts = c(0))
  }
  
  uni_rts <- zonal(x[['sizes']], 
                   x[['uni_rts']], 
                   sum) %>%
    select(uni_rts = sizes)
  if (nrow(uni_rts) == 0) {
    uni_rts <- data.frame(uni_rts = c(0))
  }
  
  int_bg <- zonal(x[['sizes']], 
                   x[['int_bg']], 
                   sum) %>%
    select(int_bg = sizes)
  if (nrow(int_bg) == 0) {
    int_bg <- data.frame(int_bg = c(0))
  }
  
  uni_bg <- zonal(x[['sizes']], 
                   x[['uni_bg']], 
                   sum) %>%
    select(uni_bg = sizes)
  if (nrow(uni_bg) == 0) {
    uni_bg <- data.frame(uni_bg = c(0))
  }
  
  int_uni <- int_rts %>%
    cbind.data.frame(uni_rts) %>%
    cbind.data.frame(int_bg) %>%
    cbind.data.frame(uni_bg) %>%
    mutate(id = id,
...
maxar_int_uni <- map_dfr(maxar_preds, ~ custom_int_uni(.x)) %>%
  mutate(imagery = 'Maxar')

planet_int_uni <- map_dfr(planet_preds, ~ custom_int_uni(.x)) %>%
  mutate(imagery = 'Planet')

sentinel_int_uni <- map_dfr(sentinel_preds, ~ custom_int_uni(.x)) %>%
  mutate(imagery = 'Sentinel-2')

int_uni <- maxar_int_uni %>%
  rbind.data.frame(planet_int_uni) %>%
  rbind.data.frame(sentinel_int_uni) %>%
  mutate(rts_iou = int_rts/uni_rts,
         bg_iou = int_bg/uni_bg,
         mean_iou = (rts_iou + bg_iou)/2)

rm(maxar_int_uni, planet_int_uni, sentinel_int_uni)

iou_from_rast <- int_uni %>%
  group_by(imagery) %>%
  summarise(rts_iou = mean(rts_iou),
            bg_iou = mean(bg_iou),
...
sentinel_rec_pre <- recall_precision(filter(polys, imagery == 'Sentinel-2')$ground_truth, 
                           filter(polys, imagery == 'Sentinel-2')$iou, 
                           thresholds)
