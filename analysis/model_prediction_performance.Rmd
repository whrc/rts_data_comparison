...
polys <- polys %>%
  full_join(val_polys %>%
            ...
            by = c('id', 'imagery')) %>%
  mutate(
    val_area = as.numeric(st_area(val_geometry)),
    val_shape = as.numeric(
      4*pi*val_area/st_perimeter(val_geometry)^2
      ) # calculate Polsby-Popper value (shape compactness/roundness)
    )
...
```{r}
ggplot(polys,
       aes(x = rts_area, y = pred_area)) +
  geom_point()

ggplot(polys,
       aes(x = val_area, y = pred_area)) +
  geom_point()

ggplot(polys,
       aes(x = rts_shape, y = pred_shape)) +
  geom_point()

ggplot(polys,
       aes(x = val_shape, y = pred_shape)) +
  geom_point()
```
