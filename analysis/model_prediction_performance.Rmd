...
-   Fix input_data so that it contains all pixels and then figure out how to get zonal stats so that it only contains the pixels within the mask
...
```{r, include = FALSE}
input_as_df <- function(input, pred, aoi_mask) {
  
  input <- mask(input, aoi_mask)
  pred <- mask(pred, aoi_mask)
