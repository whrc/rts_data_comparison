...
-   Test different thresholds for RTS
...
```{r, include = FALSE}
pred_as_poly <- function(prediction, threshold) {
...
maxar_rts_threshold <- maxar_preds %>% 
  map_dfr(~ calc_rts_threshold(.x, seq(0.3, 0.7, by = 0.01))) %>%
  group_by(threshold) %>%
...
maxar_rts_threshold_old <- maxar_preds_old %>% 
  map_dfr(~ calc_rts_threshold(.x, seq(0.3, 0.7, by = 0.01))) %>%
  group_by(threshold) %>%
  summarise(mean_iou = mean(mean_iou)) %>%
  ungroup() %>%
...
maxar_pred_polys <- maxar_preds %>%
  map_dfr(~ pred_as_poly(.x, maxar_rts_threshold)) %>%
  mutate(imagery = 'Maxar')

maxar_pred_polys_old <- maxar_preds_old %>%
  map_dfr(~ pred_as_poly(.x, maxar_rts_threshold_old)) %>%
  mutate(imagery = 'Maxar')

planet_pred_polys <- planet_preds %>%
  map_dfr(~ pred_as_poly(.x, planet_rts_threshold)) %>%
  mutate(imagery = 'Planet')

sentinel_pred_polys <- sentinel_preds %>%
  map_dfr(~ pred_as_poly(.x, sentinel_rts_threshold)) %>%
  mutate(imagery = 'Sentinel-2')
```
