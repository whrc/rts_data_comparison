...
performance_colors <- c('#00CCFF', 'gray20', '#CC0000')
```
...
maxar_pred_dir <- 'maxar20230601-161139_04062023'
planet_pred_dir <- 'planet20230604-211027_p04062023'
sentinel_pred_dir <- 'sentinel20230605-000543_p05062023'

gd_pred_folders <- drive_ls(paste0(gd_dir, '/deprecated/predictions')) %>%
...
if (length(maxar_pred_files) == length(planet_pred_files) & 
  ...
  sentinel_pred_files <- drive_ls(gd_pred_folders %>%
                                    filter(str_detect(name, 'sentinel')),
                                  pattern = 'tif$') %>%
    arrange(name)
...
```{r, include = FALSE}
### remove this line when the newest version has been uploaded to google drive
planet_pred_files <- list.files('./data/planet/predictions-planet20230604-211027_p04062023',
                                full.names = TRUE)
###
...
```{r, include = FALSE}
### remove this line when the newest version has been uploaded to google drive
planet_input_files <- list.files('./data/planet',
                                full.names = TRUE,
                                recursive = TRUE)
planet_input_files <- planet_input_files[str_detect(planet_input_files, 'valtest')]
###
...
```{r}
rts_performance <- polys %>%
  select(id, imagery, rts_area, name_long, ep_ci_size) %>%
  mutate(ep_ci_size_high = case_when(ep_ci_size == 'High' ~ 1,
                                     TRUE ~ 0),
         ep_ci_size_expected = case_when(ep_ci_size == 'Expected' ~ 1,
                                     TRUE ~ 0),
         ep_ci_size_low = case_when(ep_ci_size == 'Low' ~ 1,
                                     TRUE ~ 0)) %>%
  group_by(id) %>%
  summarise(High = sum(ep_ci_size_high),
            Expected = sum(ep_ci_size_expected),
            Low = sum(ep_ci_size_low)) %>%
  pivot_longer(High:Low, names_to = 'ep_ci_size', values_to = 'n') %>%
  group_by(id) %>%
  mutate(ep_ci_size = factor(ep_ci_size,
                             levels = c('High', 'Expected', 'Low')),
         group_max = max(n),
         group_ep_max = ep_ci_size[which(n == max(n))]) %>%
  ungroup() %>%
  mutate(group_ep_2nd = case_when(n == 1 ~ ep_ci_size,
                                  TRUE ~ NA),
         plotting_order = case_when(group_max == 2 & group_ep_max == 'High' & group_ep_2nd == 'Low' ~ 1,
                                    group_max == 3 & group_ep_max == 'High' ~ 2,
                                    group_max == 2 & group_ep_max == 'High' & group_ep_2nd == 'Expected' ~ 3,
                                    group_max == 2 & group_ep_max == 'Expected' & group_ep_2nd == 'High' ~ 4,
                                    group_max == 3 & group_ep_max == 'Expected' ~ 5,
                                    group_max == 2 & group_ep_max == 'Expected' & group_ep_2nd == 'Low' ~ 6,
                                    group_max == 2 & group_ep_max == 'Low' & group_ep_2nd == 'Expected' ~ 7,
                                    group_max == 3 & group_ep_max == 'Low' ~ 8)) %>%
...
```{r, warning = FALSE}
ggplot(rts_performance, 
       aes(x = n, color = ep_ci_size)) +
  geom_freqpoly(bins = 4) +
  scale_x_continuous(name = '# Imagery Sources',
                     limits = c(0, 3)) +
  scale_y_continuous(name = 'RTS Feature Count') +
...
ggplot(rts_performance %>%
         filter(n != 0), 
       aes(x = ep_ci_size, y = id, color = ep_ci_size, size = n)) +
  geom_point() +
  scale_x_discrete(name = 'Performance') +
  scale_y_discrete(
    name = 'RTS Features',
    limits = rev
    ) +
...
model_output <- zonal_stats %>%
  select(imagery:sr_sd, ep_ci_size) %>%
...
zonal_stats_contrasts <- model_output %>%
  ...
  filter(p_value <= 0.1) %>%
  separate_wider_delim(contrast, delim = ' - ', names = c('x_min', 'x_max'), cols_remove = FALSE)
```
...
zonal_stats_limits <- zonal_stats %>%
  select(imagery:sr_sd, ep_ci_size) %>%
  pivot_longer(r_mean:sr_sd,  names_to = 'variable', values_to = 'value') %>%
group_by(imagery, ep_ci_size, rts_class, variable) %>%
