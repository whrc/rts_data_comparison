...
- makes maps of predictions on top of non-RGB layers
...
zonal_stats_variables <- zonal_stats_contrasts %>%
  filter(p_value <= 0.1 & !(variable %in% c('lum_mean', 'lum_sd'))) %>%
  pull(variable) %>%
  unique()
# zonal_stats_variables <- c('r_mean',
#                            'g_mean',
#                            'b_mean',
#                            'sr_mean',
#                            'nir_mean',
...
zonal_stats_contrasts_mean <- zonal_stats_contrasts %>%
  ...
  select(imagery, rts_class, variable, contrast, estimate, se, p_value, star_label, variable_num, xmin, xmax) %>%
  left_join(zonal_stats_mean %>%
              mutate(upr = value_mean + value_se) %>%
              filter(upr == max(upr),
...
ggplot(zonal_stats_mean,
  ...
  geom_rect(data = tibble(xmin = c(0.5, 6.5, 11.5),
                          xmax = c(4.5, 7.5, 12.5),
                          ymin = -Inf,
                          ymax = Inf),
            aes(xmin = xmin,
  ...
  geom_signif(data = zonal_stats_contrasts_mean,
              aes(x = xmin, y = y, xend = xmax, yend = y,
                  annotation = star_label, group = group),
              stat="identity",
              manual = TRUE,
              inherit.aes = FALSE,
              na.rm = TRUE) +
  scale_x_continuous(
    ...
    labels = levels(zonal_stats_mean$variable),
