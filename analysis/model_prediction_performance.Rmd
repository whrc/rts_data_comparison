...
-   Finalize RTS sample figure. Put IoU label in plot instead of as a facet label. Add imagery facet labels. Adjust order of predictions to be grouped by what kind of example I am using the prediction for.
...
plot_individual_prediction <- function(imagery_type, raster_data, vector_data, dims) {
  ...
  figure <- ggplot(data = vector_data %>%
               filter(imagery == imagery_type)) +
...
plot_prediction_2 <- function(pid, imagery_types, dims) {
  ...
  current_dims <- current_rgb %>%
    summarise(x_min = mean(x) - dims$x_spread_max/2,
              x_max = mean(x) + dims$x_spread_max/2,
              y_min = mean(y) - dims$y_spread_max/2,
              y_max = mean(y) + dims$y_spread_max/2)
...
rts_maps_subset <- map(
  pred_ids[which(pred_ids %in% id_subset)],
